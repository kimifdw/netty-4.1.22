 ...
 * integer parameter.将HTTP查询字符串分割为路径字符串和键值参数对。此解码器仅供一次使用。为每个URI创建一个新实例:
 *
...
public class QueryStringDecoder {
     ...
     * assume that the query string is encoded in UTF-8.创建一个新的解码器来解码指定的URI。解码器将假设查询字符串是用UTF-8编码的。
     */
    ...
    /**
     * Returns the uri used to initialize this {@link QueryStringDecoder}.返回用于初始化这个QueryStringDecoder的uri。
     */
    public String uri() {
        return uri;
    }
    ...
    /**
     * Returns the decoded path string of the URI.返回URI的解码路径字符串。
     */
    public String path() {
        if (path == null) {
    ...
    /**
     * Returns the decoded key-value parameter pairs of the URI.返回URI的解码键值参数对。
     */
    ...
    /**
     * Returns the raw path string of the URI.返回URI的原始路径字符串。
     */
    ...
    /**
     * Returns raw query string of the URI.返回URI的原始查询字符串。
     */
     ...
     * escape sequence.
     * 解码由浏览器编码的URL的一部分。
    这相当于使用UTF-8字符集调用decodeComponent(String, Charset)(建议遵守RFC 3986，第2节)。
     */
     ...
     * escape sequence.
     * 解码由浏览器编码的URL的一部分。
    字符串将按照第2节RFC 3986进行编码。这是JavaScript函数encodeURI和encodeURIComponent使用的编码，但不是转义。例如，在这种编码中，e(在Unicode U+00E9或UTF-8 0xC3 0xA9中)编码为%C3%A9或%C3%A9。
    这与调用URLDecoder.decode(String, String)本质上是等价的，只是它比URLDecoder.decode(String, String)快两倍以上，并且为GC生成的垃圾更少。实际上，如果没有要解码的内容，这个函数就不会分配任何内存，而是返回参数本身。
     */
