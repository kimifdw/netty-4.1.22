 ...
 *
 * @see HttpServerCodec
 * HttpRequestEncoder和HttpResponseDecoder的组合，使客户端HTTP实现更容易。HttpClientCodec为头和连接请求提供了额外的状态管理，这是HttpResponseDecoder所缺少的。请参考HttpResponseDecoder，了解需要对HEAD和CONNECT做哪些额外的状态管理，以及为什么HttpResponseDecoder不能自己处理它。如果通道被关闭，并且缺少响应，则抛出一个PrematureChannelClosureException。
 */
public final class HttpClientCodec extends CombinedChannelDuplexHandler<HttpResponseDecoder, HttpRequestEncoder>
        implements HttpClientUpgradeHandler.SourceCodec {

    /** A queue that is used for correlating a request and a response. 用于关联请求和响应的队列。*/
     ...
     * 创建一个带有默认解码器选项的新实例(maxInitialLineLength (4096}， maxHeaderSize(8192)和maxChunkSize(8192))。
     */
    ...
    /**
     * Prepares to upgrade to another protocol from HTTP. Disables the {@link Encoder}.准备从HTTP升级到另一个协议。禁用HttpClientCodec.Encoder。
     */
    @Override
     ...
     * the pipeline.从HTTP升级到另一个协议。消除了HttpClientCodec。译码器和HttpClientCodec。编码器的管道。
     */
    @Override
