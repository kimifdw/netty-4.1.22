...
public class HttpPostRequestDecoder implements InterfaceHttpPostRequestDecoder {
    ...
    /**
     * Check if the given request is a multipart request 检查给定的请求是否为多部分请求
    ...
    /**
     * Split the very first line (Content-Type value) in 3 Strings 将第一行(Content-Type值)分成3个字符串
     *
     ...
     * enough data are available (need more chunks) 当尝试以块格式从请求中读取数据时出现异常，并且没有足够的数据可用(需要更多块)
     */
