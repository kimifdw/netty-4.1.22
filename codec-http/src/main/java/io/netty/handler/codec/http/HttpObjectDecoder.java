 ...
 *
 ...
 起始行的最大长度(例如如果初始行长度超过这个值，就会引发TooLongFrameException异常。
 maxHeaderSize
 所有标题的最大长度。如果每个标头的长度之和超过这个值，就会引发TooLongFrameException异常。
 maxChunkSize
 内容或每个块的最大长度。如果内容长度(或每个块的长度)超过此值，内容或块将被分割为多个HttpContents，其最大长度为maxChunkSize。
 分块内容
 如果HTTP消息的内容大于maxChunkSize，或者HTTP消息的传输编码被“分块”，该解码器将为每个HTTP消息生成一个HttpMessage实例及其后续HttpContents，以避免过度的内存消耗。例如，以下HTTP消息:

 触发器HttpRequestDecoder生成3个对象:
 一个HttpRequest,
 第一个HttpContent的内容是'abcdefghijklmnopqrstuvwxyz'，
 第二个LastHttpContent的内容是“1234567890abcdef”，这标志着内容的结束。
 如果为了方便起见，您不愿意自己处理HttpContents，那么可以在ChannelPipeline中在这个解码器之后插入HttpObjectAggregator。但是，请注意，您的服务器可能不如没有聚合器那样高效。
 可扩展性
 请注意，这个解码器被设计用来实现从HTTP派生的协议，例如RTSP和ICAP。要实现此类派生协议的解码器，请扩展该类并正确实现所有抽象方法。
 */
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
     ...
     * 如果服务器切换到不同于HTTP/1.0或HTTP/1.1的协议，例如HTTP/2或Websocket，则返回true。如果升级发生在不同的层，例如通过TLS从HTTP/1.1升级到HTTP/1.1，则返回false。
     */
