...
public class HttpPostMultipartRequestDecoder implements InterfaceHttpPostRequestDecoder {
    ...
    /**
     * Does the last chunk already received 最后一个块是否已经收到
     */
    ...
    /**
     * Body HttpDatas current position 身体HttpDatas当前位置
     */
    private int bodyListHttpDataRank;

    /**
     * If multipart, this is the boundary for the global multipart 如果是multipart，这就是全局multipart的边界
     */
    private String multipartDataBoundary;

    /**
     * If multipart, there could be internal multiparts (mixed) to the global
     * multipart. Only one level is allowed. 如果是multipart，则全局multipart可能有内部的multiparts(混合)。只允许一个级别。
     */
    ...
    /**
     * The current FileUpload that is currently in decode process 当前正在解码过程中的文件上传
     */
    private FileUpload currentFileUpload;

    /**
     * The current Attribute that is currently in decode process 当前正在解码过程中的当前属性
     */
     ...
     * was called it is not possible to operate on it anymore.销毁HttpPostMultipartRequestDecoder并释放所有it资源。在这个方法被调用之后，就不可能再对它进行操作了。
     */
    @Override
    public void destroy() {
        checkDestroyed();
     ...
     * Mixed mode 在混合模式下删除两个文件上传之间需要清除的所有属性
     */
     ...
     *             Need more chunks and reset the {@code readerIndex} to the previous
     *             value
     *             将一行读到——分隔符或——分隔符，如果存在，则CRLF或LF将一行读到——分隔符或——分隔符，如果存在，则CRLF或LF。注意，CRLF或LF对于打开分隔符(——分隔符)是强制性的，但对于关闭分隔符(——分隔符)则不是强制性的，因为在这种情况下，有些客户端不包括CRLF。
     */
     ...
     *             value
     *             读取一行到——分隔符或——分隔符，如果存在CRLF或LF。注意，CRLF或LF对于打开分隔符(——分隔符)是强制性的，但对于关闭分隔符(——分隔符)则不是强制性的，因为在这种情况下，有些客户端不包括CRLF。
     */
