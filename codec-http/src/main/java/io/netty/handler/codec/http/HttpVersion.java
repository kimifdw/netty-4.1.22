...
public class HttpVersion implements Comparable<HttpVersion> {
     ...
     * returned.返回与指定协议版本字符串匹配的现有或新HttpVersion实例。如果指定的文本等于“HTTP/1.0”，将返回HTTP_1_0。如果指定的文本等于“HTTP/1.1”，将返回HTTP_1_1。否则，将返回一个新的HttpVersion实例。
     */
     ...
     *        使用指定的版本字符串创建新的HTTP版本。除非要实现从HTTP派生的协议(如RTSP和ICAP)，否则不需要创建新实例。
     */
     ...
     * <a href="http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol">RTSP</a> and
     * <a href="http://en.wikipedia.org/wiki/Internet_Content_Adaptation_Protocol">ICAP</a>
     * 使用指定的协议名称和版本号创建一个新的HTTP版本。除非要实现从HTTP派生的协议(如RTSP和ICAP)，否则不需要创建新实例
     *
     * @param keepAliveDefault
     *        {@code true} if and only if the connection is kept alive unless
     *        the {@code "Connection"} header is set to {@code "close"} explicitly.
    ...
    /**
     * Returns the name of the protocol such as {@code "HTTP"} in {@code "HTTP/1.0"}.返回协议的名称，例如“HTTP/1.0”中的“HTTP”。
     */
    ...
    /**
     * Returns the name of the protocol such as {@code 1} in {@code "HTTP/1.0"}.返回协议的名称，例如“HTTP/1.0”中的1。
     */
    ...
    /**
     * Returns the name of the protocol such as {@code 0} in {@code "HTTP/1.0"}.返回协议的名称，例如“HTTP/1.0”中的0。
     */
    ...
    /**
     * Returns the full protocol version text such as {@code "HTTP/1.0"}.返回完整的协议版本文本，例如“HTTP/1.0”。
     */
    public String text() {
        return text;
    }
     ...
     * the {@code "Connection"} header is set to {@code "close"} explicitly.当且仅当连接保持活动时返回true，除非将“连接”标头显式设置为“关闭”。
     */
