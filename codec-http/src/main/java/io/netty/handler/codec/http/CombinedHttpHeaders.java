 ...
 * 将为相同的标头添加多个值，就像单个标头一样，使用逗号分隔的值列表。
 请参阅RFC 7230节，3.2.2。
 */
public class CombinedHttpHeaders extends DefaultHttpHeaders {
    ...
    private static final class CombinedHttpHeadersImpl
        ...
        /**
         * Escapes comma separated values (CSV).转义逗号分隔值(CSV)。
         *
         * @param <T> The type that a concrete implementation handles
         */
        private interface CsvValueEscaper<T> {
            /**
             * Appends the value to the specified {@link StringBuilder}, escaping if necessary.将该值追加到指定的StringBuilder，如果需要则进行转义。
             *
