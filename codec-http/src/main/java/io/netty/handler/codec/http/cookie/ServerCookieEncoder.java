 ...
 * RFC6265兼容cookie编码器将用于服务器端，因此将发送一些字段(版本通常被忽略)。当Netty的Cookie合并过期并将MaxAge合并到单个字段时，只发送Max-Age字段。注意，多个cookie必须作为单独的“Set-Cookie”头文件发送。
 */
public final class ServerCookieEncoder extends CookieEncoder {
     ...
     * 验证名称和值字符的严格编码器位于RFC6265中定义的有效范围内，并且(对于接受多个cookie的方法)只有一个cookie使用任何给定名称进行编码。(如果多个cookie有相同的名称，最后一个是编码的。)
     */
    public static final ServerCookieEncoder STRICT = new ServerCookieEncoder(true);

    /**
     * Lax instance that doesn't validate name and value, and that allows multiple
     * cookies with the same name.松散实例，不验证名称和值，并允许多个cookie具有相同的名称。
     */
    ...
    /**
     * Encodes the specified cookie name-value pair into a Set-Cookie header value.将指定的cookie名-值对编码为Set-Cookie标头值。
     *
    ...
    /** Deduplicate a list of encoded cookies by keeping only the last instance with a given name.通过只保留最后一个具有给定名称的实例，可以对编码的cookie列表进行重复。
     *
    ...
    /**
     * Batch encodes cookies into Set-Cookie header values.批量将cookie编码为Set-Cookie头值。
     *
