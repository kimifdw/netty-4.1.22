 ...
 *  </pre>
 * </blockquote>
 * 将100个CONTINUE HttpResponse发送给httprequest，其中包含一个“expect: 100- CONTINUE”头文件。它应该只用于不安装HttpObjectAggregator的应用程序。
 默认情况下，它接受所有期望。
 因为HttpServerExpectContinueHandler期待httprequest后，它应该添加HttpServerCodec，但在任何其他处理程序可能发送HttpResponse之前。
 */
public class HttpServerExpectContinueHandler extends ChannelInboundHandlerAdapter {
     ...
     * request should be rejected. See {@link #rejectResponse(HttpRequest)}.为定义期望的httprequest生成一个HttpResponse。如果请求应该被拒绝，则返回null。看到rejectResponse(HttpRequest)。
     */
    ...
    /**
     * Returns the appropriate 4XX {@link HttpResponse} for the given {@link HttpRequest}.返回给定HttpRequest的相应4XX HttpResponse。
     */
