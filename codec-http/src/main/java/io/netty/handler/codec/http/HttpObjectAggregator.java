 ...
 * @see HttpServerCodec
 * 一个ChannelHandler，它将HttpMessage及其后续的HttpContents聚合到单个FullHttpRequest或FullHttpResponse(取决于它是否用于处理请求或响应)，而没有后续的HttpContents。当您不想处理传输编码为'chunked'的HTTP消息时，它非常有用。如果要处理响应，请在ChannelPipeline中HttpResponseDecoder之后插入这个处理程序;如果要处理请求，则在ChannelPipeline中HttpRequestDecoder和HttpResponseEncoder之后插入这个处理程序。
 */
