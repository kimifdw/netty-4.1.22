 ...
 * </ul>
 * 默认工厂给予属性和文件上传根据构造函数。
 根据构造函数，属性和FileUpload可以:
 MemoryAttribute,DiskAttribute或MixedAttribute
 MemoryFileUpload,DiskFileUpload或MixedFileUpload
 */
public class DefaultHttpDataFactory implements HttpDataFactory {

    /**
     * Proposed default MINSIZE as 16 KB.建议的默认MINSIZE为16kb。
     */
    public static final long MINSIZE = 0x4000;
    /**
     * Proposed default MAXSIZE = -1 as UNLIMITED 建议的默认MAXSIZE = -1为无限制
     */
     ...
     * 保存所有HttpDatas，直到调用清理方法。我们需要使用IdentityHashMap，因为不同的请求可能是相等的。看到DefaultHttpRequest。hashCode DefaultHttpRequest.equals。同样，在删除数据项时，我们需要检查它们的标识，因为不同的数据项可能是相等的。
     */
     ...
     * The type will be Mixed.如果小于默认大小(16KB)， HttpData将在内存中。类型将是混合的。
     */
    ...
    /**
     * HttpData will be always on Disk if useDisk is True, else always in Memory if False 如果useDisk为真，HttpData将始终在磁盘上，如果useDisk为假，则始终在内存中
     */
     ...
     * will be in memory. The type will be Mixed.如果文件的大小大于minSize, HttpData将在磁盘上，否则它将在内存中。类型将是混合的。
     */
