 ...
 *  </pre>
 * </blockquote>
 * 在适当的时候，HttpServerKeepAliveHandler帮助关闭持久连接。
 如果服务器通道能够处理持久连接，那么它应该设置正确的“连接”头。HttpServerKeepAliveHandler将自动关闭与关闭连接的客户机请求相对应的任何LastHttpContent的通道，或者与LastHttpContent相关联的HttpResponse请求关闭连接，或者没有自定义消息长度。
 由于HttpServerKeepAliveHandler期待HttpObjects将在HttpServerCodec之后添加，但在任何其他可能发送HttpResponse的处理程序之前。
 */
public class HttpServerKeepAliveHandler extends ChannelDuplexHandler {
     ...
     * 只有当客户端能够检测到消息何时结束而不依赖连接被关闭时，Keep-alive才有效。
     */
