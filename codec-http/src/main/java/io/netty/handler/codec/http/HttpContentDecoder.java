 ...
 * 解码接收到的HttpRequest和HttpContent的内容。原始内容被由EmbeddedChannel(由newContentDecoder(String)创建)解码的新内容所替代。解码完成后，' content - encoding '头值被设置为目标内容编码，由getTargetContentEncoding(String)返回。此外，' content - length '标头被更新为解码内容的长度。如果解码器不支持原始内容编码，newContentDecoder(String)应该返回null，这样就不会发生解码(即传递)。
 请注意，这是一个抽象类。您必须扩展这个类并正确地实现newContentDecoder(String)，以使这个类具有功能性。例如，请参考HttpContentDecompressor的源代码。
 这个处理程序必须放在管道中的HttpObjectDecoder之后，这样这个处理程序就可以在HttpObjectDecoder将ByteBufs转换为HTTP请求之后拦截HTTP请求。
 */
public abstract class HttpContentDecoder extends MessageToMessageDecoder<HttpObject> {
     ...
     * content encoded in the specified <tt>contentEncoding</tt>.返回一个新的EmbeddedChannel，用于解码以指定内容编码的HTTP消息内容。
     *
     ...
     * most decoders.返回解码内容的预期内容编码。这个getMethod默认返回“identity”，这是大多数解码器的情况。
     *
     * @param contentEncoding the value of the {@code "Content-Encoding"} header
