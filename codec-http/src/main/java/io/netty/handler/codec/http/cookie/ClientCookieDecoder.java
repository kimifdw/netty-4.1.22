 ...
 * eventually sent back to the Origin server as is.一个RFC6265兼容cookie解码器用于客户端。它将存储原始值包装在Cookie.setWrap(布尔值)中的方式，以便最终可以按原样发送回原始服务器。
 *
...
public final class ClientCookieDecoder extends CookieDecoder {
     ...
     * defined in RFC6265 验证名称和值字符的严格编码器位于RFC6265中定义的有效范围内
     */
    public static final ClientCookieDecoder STRICT = new ClientCookieDecoder(true);

    /**
     * Lax instance that doesn't validate name and value 不验证名称和值的松散实例
     */
    ...
    /**
     * Decodes the specified Set-Cookie HTTP header value into a {@link Cookie}.将指定的Set-Cookie HTTP头值解码为Cookie。
     *
    ...
    private static class CookieBuilder {
         ...
         * cookie name/value. Unknown attribute names are silently discarded.解析并存储一个键值对。第一个被认为是cookie名/值。未知的属性名会被静静地丢弃。
         *
         * @param keyStart
