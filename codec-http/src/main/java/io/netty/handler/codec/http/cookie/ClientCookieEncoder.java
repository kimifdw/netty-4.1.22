 ...
 * RFC6265兼容cookie编码器将用于客户端，因此只发送名称=值对。注意，多个Cookie应该在一个“Cookie”头中一次性发送。
 */
public final class ClientCookieEncoder extends CookieEncoder {
     ...
     * 验证名称和值字符的严格编码器位于有效范围内，并且(对于接受多个cookie的方法)按照路径长度递减顺序对cookie进行排序，如RFC6265中所指定的那样。
     */
     ...
     * cookies in the order in which they were given.不验证名称和值的松散实例，并且(对于接受多个cookie的方法)保持cookie的顺序。
     */
    ...
    /**
     * Encodes the specified cookie into a Cookie header value.将指定的cookie编码为cookie头值。
     *
     * @param name
     *            the cookie name
     * @param value
     ...
     * order of creation time, as recommended by RFC 6265.按照RFC 6265的建议，按照路径长度的递减顺序对cookie进行排序，按照创建时间的递增顺序进行排序，从而打破连接。
     */
