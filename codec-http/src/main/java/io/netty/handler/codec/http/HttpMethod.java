 ...
 * <a href="http://en.wikipedia.org/wiki/Internet_Content_Adaptation_Protocol">ICAP</a>.HTTP或其派生协议(如RTSP和ICAP)的请求方法。
 */
public class HttpMethod implements Comparable<HttpMethod> {
     ...
     * retrieval.OPTIONS方法表示对请求/响应链上由请求uri标识的可用通信选项的信息的请求。此方法允许客户端确定与资源相关的选项和/或需求，或服务器的功能，而无需暗示资源操作或启动资源检索。
     */
     ...
     * of the process, unless that text happens to be the output of the process.GET方法意味着检索由请求uri标识的任何信息(以实体的形式)。如果请求uri指的是数据生成过程，则应该将生成的数据作为响应中的实体返回，而不是过程的源文本，除非该文本恰好是过程的输出。
     */
    public static final HttpMethod GET = new HttpMethod("GET");

    /**
     * The HEAD method is identical to GET except that the server MUST NOT return a message-body
     * in the response.HEAD方法与GET方法相同，只是服务器不能在响应中返回消息体。
     */
     ...
     * Request-Line.POST方法用于请求原始服务器接受包含在请求中的实体，作为请求行中由请求uri标识的资源的新从属关系。
     */
    public static final HttpMethod POST = new HttpMethod("POST");

    /**
     * The PUT method requests that the enclosed entity be stored under the supplied Request-URI.PUT方法请求将所包含的实体存储在提供的请求uri下。
     */
    public static final HttpMethod PUT = new HttpMethod("PUT");

    /**
     * The PATCH method requests that a set of changes described in the
     * request entity be applied to the resource identified by the Request-URI.补丁方法请求将请求实体中描述的一组更改应用于由请求uri标识的资源。
     */
    public static final HttpMethod PATCH = new HttpMethod("PATCH");

    /**
     * The DELETE method requests that the origin server delete the resource identified by the
     * Request-URI.DELETE方法请求原始服务器删除由请求uri标识的资源。
     */
    public static final HttpMethod DELETE = new HttpMethod("DELETE");

    /**
     * The TRACE method is used to invoke a remote, application-layer loop- back of the request
     * message.跟踪方法用于调用请求消息的远程应用程序层循环。
     */
    public static final HttpMethod TRACE = new HttpMethod("TRACE");

    /**
     * This specification reserves the method name CONNECT for use with a proxy that can dynamically
     * switch to being a tunnel 此规范保留方法名称CONNECT，以便与一个可以动态切换为隧道的代理一起使用
     */
     ...
     * will be returned.  Otherwise, a new instance will be returned.返回由指定名称表示的HttpMethod。如果指定的名称是标准HTTP方法名称，将返回一个缓存的实例。否则，将返回一个新实例。
     */
