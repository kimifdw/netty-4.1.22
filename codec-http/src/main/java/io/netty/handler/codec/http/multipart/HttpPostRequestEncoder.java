 ...
 *
 这个编码器将有助于将表单的请求编码为POST。
 根据RFC 7231, POST、PUT和OPTIONS允许有一个body。此编码器将广泛支持除跟踪以外的所有方法，因为RFC注释为GET、DELETE、HEAD和CONNECT:(用这些方法之一替换XXX)
 XXX请求消息中的有效负载没有定义的语义;在XXX请求上发送有效负载体可能会导致一些现有实现拒绝该请求。
 相反，对于TRACE method, RFC说:
 “客户端不能在跟踪请求中发送消息主体。”
 */
public class HttpPostRequestEncoder implements ChunkedInput<HttpContent> {

    /**
     * Different modes to use to encode form data.用于编码表单数据的不同模式。
     */
    public enum EncoderMode {
         ...
         * 最适合大多数人的遗留模式。众所周知，它与OAUTH不合作。使用OAUTH RFC3986。W3C表单推荐提交post表单数据。
         */
        RFC1738,

        /**
         * Mode which is more new and is used for OAUTH 更多的新模式，用于OAUTH
         */
        RFC3986,
         ...
         *   HTML5规范不允许在多部分/表单数据请求中使用混合模式。更具体地说，这意味着以相同名称提交的更多文件将不会使用混合模式进行编码，而是将被视为不同的字段。参考:http://www.w3.org/TR/html5/forms.html包含多部分的表单数据
         */
        HTML5
    }
    ...
    /**
     * Factory used to create InterfaceHttpData 用于创建InterfaceHttpData的工厂
     */
    ...
    /**
     * The final Multipart List of InterfaceHttpData including encoding InterfaceHttpData的最终多部分列表，包括编码
     */
    final List<InterfaceHttpData> multipartHttpDatas;

    /**
     * Does this request is a Multipart request 这个请求是多部分请求吗
     */
    private final boolean isMultipart;

    /**
     * If multipart, this is the boundary for the flobal multipart 如果是multipart，这是flobal multipart的边界
     */
    String multipartDataBoundary;

    /**
     * If multipart, there could be internal multiparts (mixed) to the global multipart. Only one level is allowed. 如果是multipart，则全局multipart可能有内部的multiparts(混合)。只允许一个级别。
     */
    String multipartMixedBoundary;
    /**
     * To check if the header has been finalized 检查标头是否已最终确定
     */
    ...
    /**
     * Does the last non empty chunk already encoded so that next chunk will be empty (last chunk) 最后一个非空块是否已经编码，以便下一个块将为空(最后一个块)
     */
    ...
    /**
     * The current FileUpload that is currently in encode process 当前正在编码过程中的文件上传
     */
    private FileUpload currentFileUpload;
    /**
     * While adding a FileUpload, is the multipart currently in Mixed Mode 在添加FileUpload时，是当前混合模式的multipart
     */
    ...
    /**
     * Init the delimiter for Global Part (Data).Init全局部分(数据)的分隔符。
     */
    ...
    /**
     * Init the delimiter for Mixed Part (Mixed).Init混合部件的分隔符。
     */
    ...
    /**
     * This getMethod returns a List of all InterfaceHttpData from body part.<br> 这个getMethod从body部分返回所有InterfaceHttpData的列表。
    ...
    /**
     * Add a series of Files associated with one File parameter 添加与一个文件参数关联的一系列文件
     *
     * @param name
     *            the name of the parameter
     * @param file
    ...
    /**
     * Add the InterfaceHttpData to the Body list 将InterfaceHttpData添加到主体列表
     *
     * @throws NullPointerException
     *             for data
     * @throws ErrorDataEncoderException
    ...
    /**
     * Iterator to be used when encoding will be called chunk after chunk 编码时使用的迭代器将在块后调用块
     */
    ...
    /**
     * From the current context (currentBuffer and currentData), returns the next HttpChunk (if possible) trying to get
     * sizeleft bytes more into the currentBuffer. This is the UrlEncoded version.从当前上下文(currentBuffer和currentData)中，返回下一个HttpChunk(如果可能的话)，尝试将更多的sizeleft字节放入currentBuffer中。这是urlencoding版本。
     *
     * @param sizeleft
     *            the number of bytes to try to get from currentData
     * @return the next HttpChunk or null if not enough bytes were found
     ...
     * in order to stop calling this getMethod.返回下一个可用的HttpChunk。调用者负责测试这个块是否是最后一个块(isLast())，以便停止调用这个getMethod。
     *
