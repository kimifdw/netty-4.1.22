 ...
 *
 一个NameResolver，它通过选择一个地址随机地选择一个地址(字符串)和解析(String, Promise)来解决InetAddress和force Round Robin的问题，如果多个由NameResolver返回。使用asAddressResolver()创建一个InetSocketAddress解析器
 */
@UnstableApi
