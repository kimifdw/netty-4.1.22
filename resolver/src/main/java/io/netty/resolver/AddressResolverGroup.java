...
/**
 * Creates and manages {@link NameResolver}s so that each {@link EventExecutor} has its own resolver instance.创建并管理NameResolvers，以便每个EventExecutor都有自己的解析器实例。
 */
@UnstableApi
public abstract class AddressResolverGroup<T extends SocketAddress> implements Closeable {
    ...
    /**
     * Invoked by {@link #getResolver(EventExecutor)} to create a new {@link AddressResolver}.getResolver(EventExecutor)调用以创建新的地址解析器。
     */
