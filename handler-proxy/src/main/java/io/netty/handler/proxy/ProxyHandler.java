...
public abstract class ProxyHandler extends ChannelDuplexHandler {
    ...
    /**
     * The default connect timeout: 10 seconds.默认连接超时:10秒。
     */
    private static final long DEFAULT_CONNECT_TIMEOUT_MILLIS = 10000;

    /**
     * A string that signifies 'no authentication' or 'anonymous'.表示“无身份验证”或“匿名”的字符串。
     */
    ...
    /**
     * Returns the name of the proxy protocol in use.返回正在使用的代理协议的名称。
     */
    public abstract String protocol();

    /**
     * Returns the name of the authentication scheme in use.返回正在使用的身份验证方案的名称。
     */
    public abstract String authScheme();

    /**
     * Returns the address of the proxy server.返回代理服务器的地址。
     */
    ...
    /**
     * Returns the address of the destination to connect to via the proxy server.返回要通过代理服务器连接到的目的地的地址。
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns {@code true} if and only if the connection to the destination has been established successfully.如果且仅当与目的地的连接已成功建立时返回true。
     */
     ...
     * or the connection attempt has failed.返回在建立到目的地的连接或连接尝试失败时被通知的未来。
     */
     ...
     * the timeout, the connection attempt will be failed.返回millis中的连接超时。如果到目的地的连接尝试未在超时内完成，则连接尝试将失败。
     */
     ...
     * the timeout, the connection attempt will be failed.设置millis中的连接超时。如果到目的地的连接尝试未在超时内完成，则连接尝试将失败。
     */
    ...
    /**
     * Adds the codec handlers required to communicate with the proxy server.添加与代理服务器通信所需的codec处理程序。
     */
    protected abstract void addCodec(ChannelHandlerContext ctx) throws Exception;

    /**
     * Removes the encoders added in {@link #addCodec(ChannelHandlerContext)}.删除添加在addCodec(ChannelHandlerContext)中的编码器。
     */
    protected abstract void removeEncoder(ChannelHandlerContext ctx) throws Exception;

    /**
     * Removes the decoders added in {@link #addCodec(ChannelHandlerContext)}.删除添加在addCodec(ChannelHandlerContext)中的解码器。
     */
     ...
     * 发送要发送到代理服务器的初始消息。该方法还启动一个超时任务，如果连接尝试在超时内没有成功，该任务将connectPromise标记为失败。
     */
    ...
    /**
     * Returns a new message that is sent at first time when the connection to the proxy server has been established.返回在建立到代理服务器的连接时第一次发送的新消息。
     *
     ...
     * 将指定的消息发送到代理服务器。使用此方法在handleResponse(ChannelHandlerContext, Object)中向代理服务器发送响应。
     */
    ...
    /**
     * Handles the message received from the proxy server.处理从代理服务器接收的消息。
     *
     ...
     * authentication scheme, proxy address, and destination address.使用诸如当前协议、身份验证方案、代理地址和目标地址等公共信息装饰指定的异常消息。
     */
