 ...
 * output {@link ByteBuf}.将用Snappy压缩编码的输入压缩为ByteBuf的输出压缩。看到时髦的格式。
 *
...
public final class Snappy {
    ...
    /**
     * Creates an appropriately sized hashtable for the given input size 为给定的输入大小创建一个适当大小的散列表
     *
     ...
     * 在提供的minIndex和maxIndex之间遍历提供的输入缓冲区，找出匹配的副本与已编写的文字值重叠的时间。
     */
     ...
     * 计算编码一个值所需的最小位数。然后，这又可以用来计算用于编码长度参数的septets或octets(酌情)的数量。
     */
     ...
     *               通过直接从输入缓冲区复制将文字写到提供的输出缓冲区。从当前的readerIndex到提供的长度取文字。
     */
    ...
    /**
     * Encodes a series of copies, each at most 64 bytes in length.编码一系列副本，每个副本最多64字节。
     *
     ...
     * read).读取长度varint(一组字节，其中较低的7位是数据，而上位是表示要读取更多字节的标志)。
     *
     ...
     * exceed the length of the chunk currently read so far.验证从压缩引用中提取的偏移量是否在偏移量允许的范围内(0 < offset < Integer.MAX_VALUE)，并且不超过当前读取的块的长度。
     *
     ...
     * 计算所提供数据的CRC32C校验和，并对所计算的校验和执行“mask”操作
     *
    ...
    /**
     * Computes the CRC32C checksum of the supplied data and performs the "mask" operation
     * on the computed checksum 计算所提供数据的CRC32C校验和，并对所计算的校验和执行“mask”操作
     *
     * @param data The input data to calculate the CRC32C checksum of
     */
     ...
     * Computes the CRC32C checksum of the supplied data, performs the "mask" operation
     * on the computed checksum, and then compares the resulting masked checksum to the
     * supplied checksum.计算所提供数据的CRC32C校验和，对所计算的校验和执行“mask”操作，然后将所得到的掩蔽校验和与所提供的校验和进行比较。
     *
     * @param expectedChecksum The checksum decoded from the stream to compare against
     * @param data The input data to calculate the CRC32C checksum of
     * @throws DecompressionException If the calculated and supplied checksums do not match
     ...
     * 来自规范:“校验和不是直接存储的，而是掩蔽的，因为校验和数据和它自己的校验和可能会有问题。屏蔽与Apache Hadoop中使用的相同:将校验和旋转15位，然后添加常量0xa282ead8(使用wraparound作为无符号整数的正常使用)。
     */
