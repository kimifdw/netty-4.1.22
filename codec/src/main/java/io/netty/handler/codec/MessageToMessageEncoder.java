 ...
 * ChannelOutboundHandlerAdapter从一个消息编码到另一个消息，例如，这里有一个实现，它将一个整数解码为一个字符串。
 * 请注意，您需要对刚刚通过的消息进行调用，如果它们是引用的类型，请保留()。这是必需的，因为MessageToMessageEncoder将调用编码消息的referencec .release()。
 */
public abstract class MessageToMessageEncoder<I> extends ChannelOutboundHandlerAdapter {
     ...
     * 如果应该处理给定的消息，则返回true。如果为false，它将被传递到ChannelOutboundHandler管道中的下一个channelboundhandler。
     *
     */
     ...
     * by this encoder.从一条消息编码到另一条消息。该方法将对每个可由编码器处理的书面消息进行调用。
     *
