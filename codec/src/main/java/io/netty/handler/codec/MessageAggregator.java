 ...
 *           将一系列消息对象聚合为单个聚合消息的抽象通道处理程序。
“一系列讯息”包括:
一个单独的起始消息，可选地包含内容的第一部分，以及。
1或更多内容信息。
聚合消息的内容将是start消息及其以下内容消息的合并内容。如果聚合器遇到isLastContentMessage(ByteBufHolder)返回true的内容消息，那么聚合器将完成聚合并生成聚合消息，并预期另一个开始消息。
 */
public abstract class MessageAggregator<I, S, C extends ByteBufHolder, O extends ByteBufHolder>
     ...
     * </pre>
     * 如果且仅当指定的消息是开始消息时返回true。通常，这个方法是用instanceof作为一个返回语句来实现的:
     返回味精instanceof MyStartMessage;
     */
     ...
     * </pre>
     * 如果且仅当指定的消息是内容消息时返回true。通常，这个方法是用instanceof作为一个返回语句来实现的:
     返回味精instanceof MyContentMessage;
     */
     ...
     * </pre>
     * 如果且仅当指定的消息是最后一个内容消息时返回true。通常，这个方法是用instanceof作为一个返回语句来实现的:
     返回味精instanceof MyLastContentMessage;

     或者使用instanceof和boolean字段检查:
     返回MyContentMessage & msg. islastfragment()的msg实例;
     */
     ...
     * 返回true，如果且仅当指定的消息已经聚合。如果该方法返回true，该处理程序将直接将消息转发给下一个处理程序。
     */
    protected abstract boolean isAggregated(I msg) throws Exception;

    /**
     * Returns the maximum allowed length of the aggregated message in bytes.返回以字节为单位的聚合消息的最大允许长度。
     */
     ...
     * 返回累积缓冲区中组件的最大数量。如果累积缓冲区中的组件数量超过此值，则累积缓冲区的组件将合并到单个组件中，包括内存副本。此属性的默认值是1024。
     */
     ...
     * 设置累计缓冲区中组件的最大数量。如果累积缓冲区中的组件数量超过此值，则累积缓冲区的组件将合并到单个组件中，包括内存副本。此属性的默认值为1024，其最小允许值为2。
     */
...
//直到请求下一个请求/响应。
     ...
     * {@code maxContentLength}. {@code false} otherwise.确定消息开始的内容长度是否已知，以及它是否大于maxContentLength。
     */
     ...
     * 如果需要，返回指定开始消息的“继续响应”。例如，该方法对于处理HTTP 100-continue头文件非常有用。
     */
     ...
     * {@link #newContinueResponse(Object, int, ChannelPipeline)} is written. {@code false} otherwise.在写入newContinueResponse(对象、int、ChannelPipeline)之后确定通道是否应该关闭。
     */
     ...
     * {@code false} otherwise.确定是否应该忽略当前请求/响应的所有对象。消息将在下次isContentMessage(对象)返回true时停止被忽略。
     */
     ...
     * 从指定的开始消息和指定的内容创建一个新的聚合消息。如果开始消息实现ByteBufHolder，那么它的内容将附加到指定的内容。这个聚合器将继续将接收到的内容附加到指定的内容。
     */
     ...
     * that the content message provides to {@code aggregated}.将指定内容消息提供的信息传输到指定的聚合消息。注意，已将指定内容消息的内容附加到指定聚合消息的内容，因此不需要这样做。使用此方法来传输内容消息提供的附加信息。
     */
    protected void aggregate(O aggregated, C content) throws Exception { }

    /**
     * Invoked when the specified {@code aggregated} message is about to be passed to the next handler in the pipeline.当指定的聚合消息即将传递给管道中的下一个处理程序时调用。
     */
