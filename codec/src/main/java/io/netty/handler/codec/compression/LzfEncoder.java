...
/**
 * Compresses a {@link ByteBuf} using the LZF format.使用LZF格式压缩ByteBuf。完整的描述见原始的LZF包和LZF格式。
 *
...
public class LzfEncoder extends MessageToByteEncoder<ByteBuf> {
     ...
     * less than {@link #MIN_BLOCK_TO_COMPRESS} will write as uncompressed.准备压缩的最小块大小。小于MIN_BLOCK_TO_COMPRESS的块将被写为未压缩。
     */
    private static final int MIN_BLOCK_TO_COMPRESS = 16;

    /**
     * Underlying decoder in use.潜在的译码器在使用中。
     */
    private final ChunkEncoder encoder;

    /**
     * Object that handles details of buffer recycling.对象，用于处理缓冲区回收的细节。
     */
     ...
     * 创建一个新的LZF编码器，使用最优的底层数据访问方法。如果可以在当前JVM上使用它，那么它将是“不安全的”实例。当实现被动态加载时，应该安全地调用这个构造函数;但是，在一些非标准平台上，可能需要使用LzfEncoder(boolean)和true param。
     */
    public LzfEncoder() {
    ...
    /**
     * Creates a new LZF encoder with specified encoding instance.使用指定的编码实例创建一个新的LZF编码器。
     *
     * @param safeInstance
     ...
     * your data flow more efficient if you know the average size of messages that you send.使用指定的编码块的总长度创建一个新的LZF编码器。如果您知道所发送的消息的平均大小，那么可以将其配置为更有效地对数据流进行编码。
     *
     * @param totalLength
    ...
    /**
     * Creates a new LZF encoder with specified settings.创建具有指定设置的新的LZF编码器。
     *
     * @param safeInstance
     *        If {@code true} encoder will use {@link ChunkEncoder} that only uses standard JDK access methods,
     *        and should work on all Java platforms and JVMs.
