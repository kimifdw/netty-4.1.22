 ...
 *           一种编码程序，它使用指定的消息编码程序将地址压缩包中的内容编码为数据压缩包。例如,
 *           注意:由于UDP数据包是无序的，您应该确保编码的消息大小不大于您的特定网络路径中的最大安全数据包大小，从而保证不存在数据包碎片。
 */
public class DatagramPacketEncoder<M> extends MessageToMessageEncoder<AddressedEnvelope<M, InetSocketAddress>> {
     ...
     *                创建一个编码器，使用指定的消息编码器将addressedene中的内容编码到DatagramPacket中。
     */
