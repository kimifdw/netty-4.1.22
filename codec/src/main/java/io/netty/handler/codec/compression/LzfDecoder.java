...
/**
 * Uncompresses a {@link ByteBuf} encoded with the LZF format.Uncompresses a ByteBuf编码的LZF格式。完整的描述见原始的LZF包和LZF格式。
 *
...
public class LzfDecoder extends ByteToMessageDecoder {
     ...
     * It is equal to {@link #chunkLength} for non compressed chunks.当前接收数据块的原始长度。对于非压缩块，它等于分块长度。
     */
     ...
     *
     创建一个新的LZF解码器，它具有最优的底层数据访问方法。如果一个实例可以在当前JVM上使用，那么它将是“不安全的”实例。当实现被动态加载时，调用此构造函数应该是安全的;但是，在一些非标准平台上，可能需要使用带有真正参数的LzfDecoder(boolean)。
     */
    public LzfDecoder() {
        this(false);
    }
    ...
    /**
     * Creates a new LZF decoder with specified decoding instance.使用指定的解码实例创建一个新的LZF解码器。
     *
     * @param safeInstance
