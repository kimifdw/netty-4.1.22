 ...
 *  *       *       *    length   *     length    *           *     *      block      *
 *  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *     * * * * * * * * * *
 *  Uncompresses a ByteBuf编码的LZ4格式。查看原始LZ4 Github项目和LZ4块格式的完整描述。由于原始的LZ4块格式不包含压缩块的大小和原始数据的大小，因此编码器使用了由Adrien Grand编写、Yann Collet(原LZ4库作者)批准的LZ4 Java库等格式。
 */
public class Lz4FrameDecoder extends ByteToMessageDecoder {
    ...
    /**
     * Compressed length of current incoming block.电流输入块的压缩长度。
     */
    private int compressedLength;

    /**
     * Decompressed length of current incoming block.解压缩当前输入块的长度。
     */
    private int decompressedLength;

    /**
     * Checksum value of current incoming block.当前输入块的校验和值。
     */
     ...
     * set to {@code true}.创建最快的LZ4解码器。注意，默认情况下，为了提高性能，每个块中的校验和头被禁用。如果性能不是问题，或者您更喜欢校验和验证所带来的安全性，请使用参数设置为true的Lz4FrameDecoder(boolean)构造函数。
     */
     ...
     * has been reached.当且仅当已到达压缩流的末尾时返回true。
     */
