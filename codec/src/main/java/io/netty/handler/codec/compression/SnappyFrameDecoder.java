 ...
 * set to {@code true}.
 * 解压用快帧格式编码的ByteBuf。看到时髦的框架格式。注意，默认情况下，为了提高性能，每个块中的校验和头被禁用。如果性能问题不大，或者您希望校验和验证带来的安全性，请使用SnappyFrameDecoder(boolean)构造函数，参数设置为true。
 */
public class SnappyFrameDecoder extends ByteToMessageDecoder {
     ...
     * {@link #SnappyFrameDecoder(boolean)} constructor.创建一个新的带有校验和验证的snappy框架译码器。为了验证该方法的有效性，请使用备用SnappyFrameDecoder(boolean)构造函数。
     */
    ...
    /**
     * Creates a new snappy-framed decoder with validation of checksums
     * as specified.使用指定的校验和创建一个新的snappy框架解码器。
     *
    ...
    /**
     * Decodes the chunk type from the type tag byte.从类型标记字节解码块类型。
     *
