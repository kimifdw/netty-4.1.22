 ...
 *        unused
 *        ByteToMessageDecoder的一种特殊变体，它支持在阻塞I/O范式中实现非阻塞解码器。
ReplayingDecoder和ByteToMessageDecoder之间最大的区别是，ReplayingDecoder允许您实现decode()和decodeLast()方法，就像所有需要的字节一样，而不是检查所需字节的可用性。例如，下面的ByteToMessageDecoder实现:
 */
public abstract class ReplayingDecoder<S> extends ByteToMessageDecoder {
    ...
    /**
     * Creates a new instance with no initial state (i.e: {@code null}).创建一个没有初始状态的新实例(i。艾凡:null)。
     */
    ...
    /**
     * Creates a new instance with the specified initial state.创建具有指定初始状态的新实例。
     */
    ...
    /**
     * Stores the internal cumulative buffer's reader position.存储内部累积缓冲区的读取位置。
     */
     ...
     * the current decoder state.存储内部累积缓冲区的读取器位置并更新当前解码器状态。
     */
    ...
    /**
     * Returns the current state of this decoder.返回此解码器的当前状态。
    ...
    /**
     * Sets the current state of this decoder.设置此解码器的当前状态。
