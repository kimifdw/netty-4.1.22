 ...
 * 一个适当的，长度限制规范霍夫曼编码长度分配器。基于R. L. Milidi'u、A. A. Pessoa和E. S. Laber提出的算法，采用适当的长度限制前缀编码，并结合Simakov Alexander的shcodec实现的其他想法。
 */
final class Bzip2HuffmanAllocator {
    ...
    /**
     * Fills the code array with extended parent pointers.用扩展的父指针填充代码数组。
     * @param array The code length array
     */
    ...
    /**
     * Finds the number of nodes to relocate in order to achieve a given code length limit.查找要重新定位的节点数，以达到给定的代码长度限制。
     * @param array The code length array
    ...
    /**
     * A final allocation pass with no code length limit.没有代码长度限制的最终分配通行证。
     * @param array The code length array
     */
    ...
    /**
     * A final allocation pass that relocates nodes in order to achieve a maximum code length limit.为了达到最大的代码长度限制而重新定位节点的最终分配通道。
     * @param array The code length array
     * @param nodesToMove The number of internal nodes to be relocated
    ...
    /**
     * Allocates Canonical Huffman code lengths in place based on a sorted frequency array.根据已排序的频率阵列来分配规范的Huffman代码长度。
