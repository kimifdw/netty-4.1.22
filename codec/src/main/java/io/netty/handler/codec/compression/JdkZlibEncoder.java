...
public class JdkZlibEncoder extends ZlibEncoder {
     ...
     * and the default wrapper ({@link ZlibWrapper#ZLIB}).使用默认的压缩级别(6)和默认包装器(ZlibWrapper.ZLIB)创建一个新的zlib编码器。
     *
     ...
     * and the default wrapper ({@link ZlibWrapper#ZLIB}).使用指定的压缩级别和默认包装器(zlibwrap . zlib)创建一个新的zlib编码器。
     *
    ...
    /**
     * Creates a new zlib encoder with the default compression level ({@code 6})
     * and the specified wrapper.使用默认的压缩级别(6)和指定的包装器创建一个新的zlib编码器。
     *
     * @throws CompressionException if failed to initialize zlib
     */
     ...
     * the preset dictionary.使用默认的压缩级别(6)和指定的预设置字典创建一个新的zlib编码器。包装器总是ZlibWrapper。ZLIB是因为它是唯一支持预设字典的格式。
     *
