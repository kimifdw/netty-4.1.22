 ...
 * them in one pass.Bzip2的编码器移动到前转换和运行长度编码[2]阶段。虽然在概念上这两个阶段是分开的，但是在一次执行它们在计算上是有效的。
 */
final class Bzip2MTFAndRLE2StageEncoder {
    /**
     * The Burrows-Wheeler transformed block.burrows - wheeler改变了块。
     */
    private final int[] bwtBlock;

    /**
     * Actual length of the data in the {@link #bwtBlock} array.bwtBlock数组中数据的实际长度。
     */
    private final int bwtLength;

    /**
     * At each position, {@code true} if the byte value with that index is present within the block,
     * otherwise {@code false}.在每个位置，如果带有该索引的字节值出现在块中，则为true，否则为false。
     */
    ...
    /**
     * The encoded alphabet size.编码字母大小。
     */
    ...
    /**
     * Performs the Move To Front transform and Run Length Encoding[1] stages.执行前向转换和运行长度编码[1]阶段的移动。
     */
    void encode() {
