 ...
 * 通过消息中的length字段的值动态地分割接收到的ByteBufs的解码器。当您解码一个具有整数头字段的二进制消息时，它特别有用，它表示消息体的长度或整个消息。
LengthFieldBasedFrameDecoder具有许多配置参数，以便用长度字段解码任何消息，这在专有的客户端-服务器协议中经常看到。这里有一些例子，可以让你知道选择做什么。
 */
