...
final class FastLz {
     ...
     * 在输入缓冲区中压缩一个数据块，并返回压缩块的大小。输入缓冲区的大小由长度指定。最小输入缓冲区大小为32。如果输入不可压缩，则返回值可能大于长度(输入缓冲区大小)。
     */
     ...
     * 解压压缩数据块并返回解压缩数据块的大小。如果发生错误，例如压缩数据被损坏或输出缓冲区不够大，则返回0(0)。解压缩是内存安全的，它保证不写输出缓冲区，而不是在outLength中指定的。
     */
