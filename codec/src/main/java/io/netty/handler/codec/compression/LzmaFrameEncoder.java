 ...
 * 使用LZMA算法压缩ByteBuf。参见LZMA和LZMA格式或LZMA SDK存档中的文档。
 */
public class LzmaFrameEncoder extends MessageToByteEncoder<ByteBuf> {
    ...
    /**
     * Underlying LZMA encoder in use.使用中的底层LZMA编码器。
     */
    ...
    /**
     * For log warning only once.只有一次日志警告。
     */
    private static boolean warningLogged;

    /**
     * Creates LZMA encoder with default settings.使用默认设置创建LZMA编码器。
     */
     ...
     * 使用指定的lc、lp、pb值和65536的中字典大小创建LZMA编码器。
     */
     ...
     * 使用指定的字典大小和lc = 3的默认值创建LZMA编码器，lp = 0, pb = 2。
     */
     ...
     * 使用指定的lc、lp、pb值和自定义字典大小创建LZMA编码器。
     */
    ...
    /**
     * Creates LZMA encoder with specified settings.创建具有指定设置的LZMA编码器。
     *
     * @param lc
    ...
    /**
     * Calculates maximum possible size of output buffer for not compressible data.计算不可压缩数据的输出缓冲区的最大可能大小。
     */
