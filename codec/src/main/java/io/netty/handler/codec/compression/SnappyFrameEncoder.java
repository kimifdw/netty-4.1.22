...
/**
 * Compresses a {@link ByteBuf} using the Snappy framing format.使用快捷的框架格式压缩ByteBuf。看到时髦的框架格式。
 *
...
public class SnappyFrameEncoder extends MessageToByteEncoder<ByteBuf> {
     ...
     * compress (instead of just emitting a literal).我们要考虑的最小压缩量。这个值是preamble +我们的快速服务将压缩的最小长度(而不是仅仅发送一个文字)。
     */
    private static final int MIN_COMPRESSIBLE_LENGTH = 18;

    /**
     * All streams should start with the "Stream identifier", containing chunk
     * type 0xff, a length field of 0x6, and 'sNaPpY' in ASCII.所有流都应该以“流标识符”开头，其中包含块类型0xff、长度字段0x6和ASCII中的“sNaPpY”。
     */
    ...
    /**
     * Calculates and writes the 4-byte checksum to the output buffer 计算并将4字节的校验和写入输出缓冲区。
     *
