...
/**
 * An encoder for the Bzip2 Huffman encoding stage.一个Bzip2霍夫曼编码阶段的编码器。
 */
final class Bzip2HuffmanStageEncoder {
    /**
     * Used in initial Huffman table generation.用于最初的哈夫曼表生成。
     */
    private static final int HUFFMAN_HIGH_SYMBOL_COST = 15;

    /**
     * The {@link Bzip2BitWriter} to which the Huffman tables and data is written.将Huffman表和数据写入到的Bzip2BitWriter。
     */
    private final Bzip2BitWriter writer;

    /**
     * The output of the Move To Front Transform and Run Length Encoding[2] stages.移动到前转换和运行长度编码[2]阶段的输出。
     */
    private final char[] mtfBlock;

    /**
     * The actual number of values contained in the {@link #mtfBlock} array.mtfBlock数组中包含的实际值数。
     */
    private final int mtfLength;

    /**
     * The number of unique values in the {@link #mtfBlock} array.mtfBlock数组中唯一值的数量。
     */
    private final int mtfAlphabetSize;

    /**
     * The global frequencies of values within the {@link #mtfBlock} array.mtfBlock数组中值的全局频率。
     */
    ...
    /**
     * Merged code symbols for each table. The value at each position is ((code length << 24) | code).每个表的合并代码符号。每个位置的值是(代码长度<< 24)|代码)。
     */
    private final int[][] huffmanMergedCodeSymbols;

    /**
     * The selectors for each segment.每个段的选择器。
     */
    ...
    /**
     * Generate a Huffman code length table for a given list of symbol frequencies.为给定的符号频率列表生成一个霍夫曼编码长度表。
     ...
     * 生成最初的Huffman代码长度表，给每个表一个不同的低成本字母表部分，总的累计频率大致相同。注意，初始表对于实际的Huffman代码生成是无效的，并且仅作为后续optimiseSelectorsAndHuffmanTables(boolean)迭代优化的种子。
     */
     ...
     *                       协同优化选择器列表和备选的Huffman表代码长度。该方法被反复调用，希望选择器的总编码大小、霍夫曼编码长度和与它们一起编码的块数据将收敛到最小值。如果数据是高度不可压缩的，那么编码的总大小很可能会发生差异(增加)。
     */
    ...
    /**
     * Assigns Canonical Huffman codes based on the calculated lengths.根据计算的长度分配规范霍夫曼编码。
     */
    ...
    /**
     * Write out the selector list and Huffman tables.写出选择器列表和霍夫曼表。
     */
    ...
    /**
     * Writes out the encoded block data.写入已编码的块数据。
     */
    ...
    /**
     * Encodes and writes the block data.对块数据进行编码和写入。
     */
