 ...
 * at a time is read from the {@link ByteBuf} when more bits are required.位读取器，允许读取单位布尔值、任意长度的位字符串(最多32位)和位对齐的32位整数。当需要更多比特时，从ByteBuf读取一个字节。
 */
class Bzip2BitReader {
    /**
     * Maximum count of possible readable bytes to check.可能要检查的可读字节的最大计数。
     */
    private static final int MAX_COUNT_OF_READABLE_BYTES = Integer.MAX_VALUE >>> 3;

    /**
     * The {@link ByteBuf} from which to read data.用来读取数据的ByteBuf。
     */
    private ByteBuf in;

    /**
     * A buffer of bits read from the input stream that have not yet been returned.从尚未返回的输入流中读取的位的缓冲区。
     */
    ...
    /**
     * Set the {@link ByteBuf} from which to read data.设置要从中读取数据的ByteBuf。
     */
