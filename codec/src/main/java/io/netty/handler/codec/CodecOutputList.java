...
/**
 * Special {@link AbstractList} implementation which is used within our codec base classes.在我们的编解码器基类中使用的特殊的抽象列表实现。
 */
final class CodecOutputList extends AbstractList<Object> implements RandomAccess {
    ...
    /**
     * Returns {@code true} if any elements where added or set. This will be reset once {@link #recycle()} was called.返回true，如果添加或设置了任何元素，将重置一次循环()。
     */
    ...
    /**
     * Recycle the array which will clear it and null out all entries in the internal storage.回收将清除它的数组并将内部存储中的所有条目空出。
     */
    void recycle() {
     ...
     * 返回给定索引上的元素。这种操作不会进行任何范围检查，因此被认为是不安全的。
     */
