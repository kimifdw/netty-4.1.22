...
/**
 * Uncompresses a {@link ByteBuf} encoded by {@link FastLzFrameEncoder} using the FastLZ algorithm.使用FastLzFrameEncoder编码的ByteBuf使用FastLZ算法解压。看到FastLZ格式。
 *
...
public class FastLzFrameDecoder extends ByteToMessageDecoder {
    ...
    /**
     * Underlying checksum calculator in use.使用的基础校验和计算器。
     */
    private final Checksum checksum;

    /**
     * Length of current received chunk of data.接收数据块的当前长度。
     */
    private int chunkLength;

    /**
     * Original of current received chunk of data.
     * It is equal to {@link #chunkLength} for non compressed chunks.原始数据。对于非压缩块，它等于分块长度。
     */
    private int originalLength;

    /**
     * Indicates is this chunk compressed or not.指示此块是否被压缩。
     */
    private boolean isCompressed;

    /**
     * Indicates is this chunk has checksum or not.指示此块是否有校验和。
     */
    private boolean hasChecksum;

    /**
     * Checksum value of current received chunk of data which has checksum.具有校验和的当前接收数据块的校验和值。
     */
    private int currentChecksum;

    /**
     * Creates the fastest FastLZ decoder without checksum calculation.创建最快的FastLZ解码器，无需校验和计算。
     */
    ...
    /**
     * Creates a FastLZ decoder with calculation of checksums as specified.创建一个FastLZ解码器，计算指定的校验和。
     *
    ...
    /**
     * Creates a FastLZ decoder with specified checksum calculator.使用指定的校验和计算器创建FastLZ解码器。
     *
     * @param checksum
