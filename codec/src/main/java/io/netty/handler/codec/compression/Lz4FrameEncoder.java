 ...
 *  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *     * * * * * * * * * *
 *
 *  使用LZ4格式压缩ByteBuf。详见原始LZ4 Github项目和LZ4块格式。由于最初的LZ4块格式不包含压缩块的大小和原始数据的大小，因此编码器使用了由Adrien Grand编写的LZ4 Java库和Yann Collet(原始LZ4库的作者)所批准的格式
 */
public class Lz4FrameEncoder extends MessageToByteEncoder<ByteBuf> {
    ...
    /**
     * Underlying compressor in use.潜在的压缩机在使用。
     */
    private final LZ4Compressor compressor;

    /**
     * Underlying checksum calculator in use.使用的基本校验和计算器。
     */
    private final ByteBufChecksum checksum;

    /**
     * Compression level of current LZ4 encoder (depends on {@link #blockSize}).当前LZ4编码器的压缩级别(取决于块大小)。
     */
    private final int compressionLevel;

    /**
     * Inner byte buffer for outgoing data. It's capacity will be {@link #blockSize}.用于输出数据的内部字节缓冲区。它的容量将是块大小。
     */
    private ByteBuf buffer;

    /**
     * Maximum size for any buffer to write encoded (compressed) data into.任何缓冲区的最大大小，以写入已编码(压缩)的数据。
     */
    private final int maxEncodeSize;

    /**
     * Indicates if the compressed stream has been finished.指示压缩流是否已完成。
     */
    private volatile boolean finished;

    /**
     * Used to interact with its {@link ChannelPipeline} and other handlers.用于与它的ChannelPipeline和其他处理程序交互。
     */
     ...
     * 根据Github上的Yann Collet的可用工作，为Java创建具有默认块大小(64 KB)和xxhash散列的最快LZ4编码器。
     */
    ...
    /**
     * Calculates compression level on the basis of block size.根据块大小计算压缩级别。
     */
     ...
     * more data.
     * 将消息编码为ByteBuf。这个方法将被调用的每个书面信息，可以由这个编码器处理。将输入缓冲区编码为输出缓冲区中的块大小。数据只有在达到块大小时才被压缩和写入;否则，它将被复制到备份缓冲区中以等待更多的数据。
     */
    @Override
    ...
    /**
     * Returns {@code true} if and only if the compressed stream has been finished.当且仅当压缩流完成时返回true。
     */
     ...
     *
     * The returned {@link ChannelFuture} will be notified once the operation completes.关闭这个Lz4FrameEncoder，然后完成编码。一旦操作完成，返回的ChannelFuture将被通知。
     */
