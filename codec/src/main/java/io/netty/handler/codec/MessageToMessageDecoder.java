 ...
 * ChannelInboundHandlerAdapter从一个消息解码到另一个消息。例如，这里有一个实现，它将一个字符串解码为一个表示字符串长度的整数。
 * 请注意，您需要调用referencecouncil .retain()对于刚刚传递的消息，如果这些消息的类型为referencecount。这是必需的，因为MessageToMessageDecoder将调用被解码的消息的referencec .release()
 *
 */
public abstract class MessageToMessageDecoder<I> extends ChannelInboundHandlerAdapter {
     ...
     * 如果应该处理给定的消息，则返回true。如果为false，它将被传递到ChannelPipeline中的下一个ChannelInboundHandler。
     */
     ...
     * by this encoder.从一条消息解码到另一条消息。这个方法将被调用的每个书面信息，可以由这个编码器处理。
     *
