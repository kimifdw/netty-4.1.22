...
/**
 * Compresses a {@link ByteBuf} using the Bzip2 algorithm.使用Bzip2算法压缩ByteBuf。看到Bzip2。
 *
...
public class Bzip2Encoder extends MessageToByteEncoder<ByteBuf> {
    ...
    /**
     * A writer that provides bit-level writes.提供位级写操作的写入器。
     */
    private final Bzip2BitWriter writer = new Bzip2BitWriter();

    /**
     * The declared maximum block size of the stream (before final run-length decoding).已声明的流的最大块大小(在最终的运行长度解码之前)。
     */
    private final int streamBlockSize;

    /**
     * The merged CRC of all blocks compressed so far.到目前为止压缩的所有块的合并CRC。
     */
    private int streamCRC;

    /**
     * The compressor for the current block.用于当前块的压缩机。
     */
    private Bzip2BlockCompressor blockCompressor;

    /**
     * (@code true} if the compressed stream has been finished, otherwise {@code false}.(@code true}如果压缩流已完成，则为false。
     */
    ...
    /**
     * Creates a new bzip2 encoder with the maximum (900,000 byte) block size.创建一个最大(90万个字节)块大小的新的bzip2编码器。
     */
    public Bzip2Encoder() {
        this(MAX_BLOCK_SIZE);
    }
    ...
    /**
     * Creates a new bzip2 encoder with the specified {@code blockSizeMultiplier}.使用指定的blockSizeMultiplier创建一个新的bzip2编码器。
    ...
    /**
     * Close current block and update {@link #streamCRC}.关闭当前块并更新streamCRC。
     */
    ...
    /**
     * Returns {@code true} if and only if the end of the compressed stream has been reached.当且仅当已到达压缩流的末尾时返回true。
     */
     ...
     * 关闭这个Bzip2Encoder，然后完成编码。一旦操作完成，返回的ChannelFuture将被通知。
     */
