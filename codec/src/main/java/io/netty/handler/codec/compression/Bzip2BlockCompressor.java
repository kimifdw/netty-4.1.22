 ...
 * 压缩并写入单个Bzip2块。块编码包括以下几个阶段:1。长度编码[1]-写(int) 2。Burrows Wheeler变换- close(ByteBuf)(通过bzip2divsort) 3。写入块头-关闭(ByteBuf) 4。移动到前变换-关闭(ByteBuf)(通过Bzip2HuffmanStageEncoder) 5。运行长度编码[2]- close(ByteBuf)(通过Bzip2HuffmanStageEncoder) 6。创建并编写Huffman表——close(ByteBuf)(通过Bzip2HuffmanStageEncoder) 7。Huffman编码和写入数据- close(ByteBuf)(通过Bzip2HuffmanStageEncoder)
 */
final class Bzip2BlockCompressor {
    ...
    /**
     * Write the Huffman symbol to output byte map.将Huffman符号写入到输出字节映射。
     */
    ...
    /**
     * Compresses and writes out the block.压缩并写出块。
     */
    ...
    /**
     * Gets available size of the current block.获取当前块的可用大小。
    ...
    /**
     * Determines if the block is full and ready for compression.确定块是否已满，并准备进行压缩。
    ...
    /**
     * Determines if any bytes have been written to the block.确定是否已将任何字节写入该块。
    ...
    /**
     * Gets the CRC of the completed block. Only valid after calling {@link #close(ByteBuf)}.获取已完成块的CRC。只有在调用close(ByteBuf)之后才有效。
