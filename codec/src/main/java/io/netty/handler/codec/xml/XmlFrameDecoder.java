 ...
 * minor modifications.
 * 基于单个XML的消息流的帧解码器。
 两个示例将更好地说明这个解码器的实际功能。
 请注意，这个解码器不适合xml流协议，如XMPP，在这里，初始xml元素会打开流，并且只在会话结束时关闭，尽管这个类可能允许对这种类型的消息流进行较小的修改。
 */
public class XmlFrameDecoder extends ByteToMessageDecoder {
     ...
     * 询问给定字节是否是xml元素名称的有效起始字符。
    请参阅W3C XML规范中的NameStartChar正式定义以获得更多信息。
     */
