...
/**
 * Compresses a {@link ByteBuf} using the FastLZ algorithm.使用FastLZ算法压缩ByteBuf。看到FastLZ格式。
 *
...
public class FastLzFrameEncoder extends MessageToByteEncoder<ByteBuf> {
    ...
    /**
     * Creates a FastLZ encoder without checksum calculator and with auto detection of compression level.创建一个FastLZ编码器没有校验和计算器和自动检测压缩级别。
     */
    ...
    /**
     * Creates a FastLZ encoder with specified compression level and without checksum calculator.创建一个具有指定压缩级别和没有校验和计算器的FastLZ编码器。
     *
     ...
     * level and calculation of checksums as specified.创建一个FastLZ编码器，自动检测压缩级别并计算指定的校验和。
     *
    ...
    /**
     * Creates a FastLZ encoder with specified compression level and checksum calculator.使用指定的压缩级别和校验和计算器创建FastLZ编码器。
     *
     * @param level supports only these values:
     *        0 - Encoder will choose level automatically depending on the length of the input buffer.
     *        1 - Level 1 is the fastest compression and generally useful for short data.
