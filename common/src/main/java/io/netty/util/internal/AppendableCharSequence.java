...
public final class AppendableCharSequence implements CharSequence, Appendable {
     ...
     * 访问此CharSequence中的值。这种方法被认为是不安全的，因为索引值被认为是合法的。只执行基础数组边界检查。
     */
     ...
     * shrink the internal char array.重置AppendableCharSequence。注意，这只会重置当前的内部位置，而不会收缩内部char数组。
     */
    public void reset() {
        pos = 0;
    }
     ...
     * Create a new {@link String} from the given start to end.
     * This method is considered unsafe as index values are assumed to be legitimate.
     * Only underlying array bounds checking is done.从给定的开始到结束创建一个新字符串。这种方法被认为是不安全的，因为索引值被认为是合法的。只执行基础数组边界检查。
     */
