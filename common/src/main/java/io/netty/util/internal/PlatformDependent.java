...
public final class PlatformDependent {
    ...
    /**
     * Return the version of Java under which this library is used.返回使用该库的Java版本。
     */
    ...
    /**
     * Returns {@code true} if and only if it is fine to enable TCP_NODELAY socket option by default.如果且仅在默认情况下启用TCP_NODELAY套接字选项，则返回true。
     */
     ...
     * 如果sun.misc返回true。在类路径中发现不安全，可用于加速直接内存访问。
     */
    ...
    /**
     * {@code true} if and only if the platform supports unaligned access.当且仅当平台支持不对齐访问时为真。
     *
     ...
     * {@code -Dio.netty.noPreferDirect} option.
     如果平台有可靠的底层直接缓冲区访问API，并且用户没有指定-Dio.netty.noPreferDirect选项，则返回true。
     */
    ...
    /**
     * Creates a new fastest {@link ConcurrentMap} implementation for the current platform.为当前平台创建一个新的最快的ConcurrentMap实现。
     */
     ...
     * 尝试释放指定的直接ByteBuffer。请注意，如果当前平台不支持此操作，或者指定的缓冲区不是直接缓冲区，此方法将不起作用。
     */
    ...
    private static final class Mpsc {
        ...
        static <T> Queue<T> newMpscQueue(final int maxCapacity) {
            ...
            // up to the next power of two and so will overflow otherwise.//计算不能大于MAX_ALLOWED_MPSC_CAPACITY的最大容量。
//这是由MpscChunkedArrayQueue实现强制的，因为它会尝试将其四舍五入
//直到2的下一个幂，否则就会溢出。
     ...
     * 创建一个安全的用于多个生产者(不同的线程)和一个消费者(一个线程!)的新队列。
     */
    ...
    /**
     * Return a {@link Random} which is not-threadsafe and so can only be used from the same thread.返回一个非线程安全的随机数，因此只能从同一个线程中使用。
     */
