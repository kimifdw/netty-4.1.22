 ...
 * 一种被编码成字符编码的字符串，其字符总是取单个字节，类似于ASCII。与使用字符数组的String不同，它在内部将内容保存在一个字节数组中，以减少内存占用，并加快数据从/到基于字节数组和字节缓冲区等字节数据结构的传输。它通常与需要CharSequence的头一起使用。
 这个类被设计成提供一个不可变字节数组，并基于这个数组的值缓存一些内部状态。但是，通过在构造或array()时不复制数组，可以提供对这个字节数组的底层访问。如果对底层字节数组做了任何更改，那么用户有责任调用arrayChanged()，以便可以重置该类的状态。
 */
public final class AsciiString implements CharSequence, Comparable<CharSequence> {
    ...
    /**
     * Create a copy of {@code value} into this instance assuming ASCII encoding.假设是ASCII编码，在这个实例中创建一个值的副本。
     */
     ...
     * equal.将指定的字符串与忽略字符大小写的字符串进行比较，如果它们相等，则返回true。
     *
     * @param string the string to compare.
     ...
     * ASCII.当忽略该情况时，如果两个CharSequence都是相等的，则返回true。这只支持8位ASCII。
     */
     ...
     * 确定集合是否包含值，并使用contentEqualsIgnoreCase(CharSequence, CharSequence)来比较值。
     */
     ...
     * @see #contentEqualsIgnoreCase(CharSequence, CharSequence)
     * 使用contentEqualsIgnoreCase(CharSequence, CharSequence)来确定a是否包含b中的所有值，以比较值。
     */
    ...
    /**
     * Returns {@code true} if the content of both {@link CharSequence}'s are equals. This only supports 8-bit ASCII.如果两个CharSequence的内容都相等，则返回true。这只支持8位ASCII。
     */
