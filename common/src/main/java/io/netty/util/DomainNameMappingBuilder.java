...
public final class DomainNameMappingBuilder<V> {
     ...
     *                 添加将指定(可选通配符)主机名映射到指定输出值的映射。主机名和值都禁止使用空值。
     ...
     * 创建不可变域命名尝试的新实例，以向结果对象添加新映射，将导致引发UnsupportedOperationException
     */
     ...
     *
     * @param <V> concrete type of value objects
     *           从域名模式到其关联值对象的不可变映射。映射由两个数组表示:键和值。关键域名模式[i]与值[i]相关联。
     */
    private static final class ImmutableDomainNameMapping<V> extends DomainNameMapping<V> {
         ...
         * 估计给定实例的字符串表示长度:est = lengthOfConstantComponents + defaultValueLength +(估计映射长度* numOfMappings) * 1.10
         *
