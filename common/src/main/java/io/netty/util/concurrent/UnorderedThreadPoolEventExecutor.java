 ...
 * EventExecutor实现，它不保证提交的任务执行的顺序，因为可能有多个线程执行这些任务。这个实现对于不需要严格排序的协议是最有用的。因为在使用它时，它不提供任何订购服务!
 */
public final class UnorderedThreadPoolEventExecutor extends ScheduledThreadPoolExecutor implements EventExecutor {
    ...
    public Future<?> shutdownGracefully(long quietPeriod, long timeout, TimeUnit unit) {
        // TODO: At the moment this just calls shutdown but we may be able to do something more smart here which 现在这就叫关机，但是我们可以做一些更聪明的事情
        //       respects the quietPeriod and timeout. 尊重quietPeriod和timeout。
//        关闭线程池
        shutdown();
    ...
    private static final class RunnableScheduledFutureTask<V> extends PromiseTask<V>
        ...
        public void run() {
            if (!isPeriodic()) {
                ...
                try {
                    // Its a periodic task so we need to ignore the return value 这是一个周期性的任务，所以我们需要忽略返回值
                    task.call();
    ...
    //这是一个特殊的包装器，我们将在execute(…)中使用它来包装提交的Runnable。这是需要的
// ScheduledThreadPoolExecutor.execute(…)将委托submit(…)，然后使用decorateTask(…)。
//这件事的问题是decoratask(…)需要确保的是，如果我们不打电话的话，我们只能做自己的装饰
//从execute(…)我们可能会创建一个无限循环，因为DefaultPromise将调用
// eventexecutorv .执行(…)当通知听众承诺时。
