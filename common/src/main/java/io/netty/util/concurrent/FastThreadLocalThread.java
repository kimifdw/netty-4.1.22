...
public class FastThreadLocalThread extends Thread {
     ...
     * 设置将线程本地变量绑定到该线程的内部数据结构。请注意，此方法仅供内部使用，因此随时可能更改。
     */
    ...
    /**
     * Returns {@code true} if {@link FastThreadLocal#removeAll()} will be called once {@link #run()} completes.如果fastthread . removeall()将在run()完成时调用，则返回true。
     */
    @UnstableApi
    ...
    /**
     * Returns {@code true} if {@link FastThreadLocal#removeAll()} will be called once {@link Thread#run()} completes.如果fastthread . removeall()将在Thread.run()完成时调用，则返回true。
     */
    @UnstableApi
