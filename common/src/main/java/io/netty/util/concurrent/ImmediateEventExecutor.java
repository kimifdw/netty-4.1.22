 ...
 * 在调用者的线程中执行可运行对象。如果执行(Runnable)是可重入的，那么它将被排队等待，直到原始的Runnable结束执行。
 从execute(Runnable)抛出的所有可抛出的对象将被吞掉并记录下来。这是为了确保所有排队的可运行对象都有机会运行。
 */
...
//    饿汉式单例模式
     ...
     * 如果我们正在执行一个Runnable，一个Runnable将被排队。这是为了防止StackOverflowError错误。
     */
    ...
    /**
     * Set to {@code true} if we are executing a runnable.如果我们正在执行一个runnable，则将其设置为true。
     */
