 ...
 * </p>
 * 将域名映射到其关联的值对象。
 DNS通配符支持主机名，因此您可以使用*.netty.io来匹配两个netty。io和downloads.netty.io。
 */
public class DomainNameMapping<V> implements Mapping<String, V> {
     ...
     * will choose the one you add first.创建一个默认的、顺序敏感的映射。如果您的主机名存在冲突，映射将选择您首先添加的主机名。
     *
     ...
     * @deprecated use {@link DomainNameMappingBuilder} to create and fill the mapping instead
     * 添加将指定(可选通配符)主机名映射到指定输出值的映射。
    DNS通配符支持主机名。例如，您可以使用*.net .io来匹配netty。io和downloads.netty.io。
     */
    @Deprecated
    ...
    /**
     * Simple function to match <a href="http://en.wikipedia.org/wiki/Wildcard_DNS_record">DNS wildcard</a>.匹配DNS通配符的简单函数。
     */
    ...
    /**
     * Returns a read-only {@link Map} of the domain mapping patterns and their associated value objects.返回域映射模式及其关联值对象的只读映射。
     */
