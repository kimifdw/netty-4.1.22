...
/**
 * Provides a mechanism to iterate over a collection of bytes.提供一种遍历字节集合的机制。
 */
public interface ByteProcessor {
    /**
     * A {@link ByteProcessor} which finds the first appearance of a specific byte.一个字节处理器，它发现一个特定字节的第一个出现。
     */
    ...
    /**
     * A {@link ByteProcessor} which finds the first appearance which is not of a specific byte.一个字节处理器，它发现第一个不是特定字节的外观。
     */
    ...
    /**
     * Aborts on a linear whitespace (a ({@code ' '} or a {@code '\t'}).在一个线性空白(a(' '或'\t')上中止。
     */
    ...
    /**
     * Aborts on a byte which is not a linear whitespace (neither {@code ' '} nor {@code '\t'}).终止一个不是线性空白的字节(既不是' '也不是'\t')。
     */
