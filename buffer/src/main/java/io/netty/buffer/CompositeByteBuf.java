 ...
 * constructor explicitly.一个虚拟缓冲区，将多个缓冲区显示为一个合并的缓冲区。建议使用ByteBufAllocator.compositeBuffer()或Unpooled.wrappedBuffer(ByteBuf…)，而不是显式地调用构造函数。
 */
public class CompositeByteBuf extends AbstractReferenceCountedByteBuf implements Iterable<ByteBuf> {
     ...
     *               添加给定ByteBuf。
    注意，这种方法不会增加合成物的书写指数。如果需要增加它，可以使用addComponent(boolean, ByteBuf)。
    ByteBuf.release()对缓冲区的所有权转移到此CompositeByteBuf。
     */
     ...
     *                添加给定ByteBufs。
    注意，这种方法不会增加合成物的书写指数。如果需要增加使用addComponents(boolean、ByteBuf[])。
    ByteBuf.release()将缓冲区中所有ByteBuf对象的所有权转移到这个CompositeByteBuf。
     */
     ...
     * @param buffer the {@link ByteBuf} to add. {@link ByteBuf#release()} ownership is transfered to this
     * {@link CompositeByteBuf}.
     *               增加给定的ByteBuf，如果递增的writerIndex为真，则增加writerIndex。ByteBuf.release()对缓冲区的所有权转移到此CompositeByteBuf。
     */
     ...
     * @param buffers the {@link ByteBuf}s to add. {@link ByteBuf#release()} ownership of all {@link ByteBuf#release()}
     * ownership of all {@link ByteBuf} objects is transfered to this {@link CompositeByteBuf}.
     *                添加给定的ByteBufs，如果递增的writerIndex为true，则增加writerIndex。ByteBuf.release()将缓冲区中所有ByteBuf对象的所有权转移到这个CompositeByteBuf。
     */
    ...
    /**
     * Precondition is that {@code buffer != null}.先决条件是缓冲区!= null。
     */
     ...
     * array of components and so affect the index etc.这应该只作为方法的最后一个操作来调用，因为这可能会调整组件的底层数组，从而影响索引等等。
     */
     ...
     * buffer will lead to an undefined behavior of this buffer.返回指定索引上的内部ByteBuf。注意，更新返回缓冲区的索引将导致此缓冲区的未定义行为。
     *
     * @param cIndex the index for which the {@link ByteBuf} should be returned
     */
