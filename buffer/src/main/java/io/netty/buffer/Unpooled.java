 ...
 * 创建一个包缓冲区
 包装缓冲区是一个缓冲区，它是一个或多个现有字节数组和字节缓冲区的视图。原始数组或缓冲区内容的任何更改都将在包装缓冲区中可见。提供了各种包装器方法，它们的名称都是wrappedBuffer()。如果您想创建一个由多个数组组成的缓冲区，以减少内存副本的数量，那么您可能需要仔细研究一下接受varargs的方法。
 创建一个拷贝缓冲
 复制的缓冲区是一个或多个现有字节数组、字节缓冲区或字符串的深层副本。与包装缓冲区不同，原始数据和复制的缓冲区之间没有共享数据。提供了各种复制方法，它们的名称都是copiedBuffer()。使用此操作将多个缓冲区合并到一个缓冲区也很方便。
 */
public final class Unpooled {
     ...
     * expands its capacity boundlessly on demand.创建一个新的big-endian Java堆缓冲区，初始容量相当小，可以根据需要无限制地扩展容量。
     */
    ...
    /**
     * Creates a new big-endian direct buffer with reasonab创建一个新的big-endian直接缓冲区，初始容量相当小，可以根据需要无限制地扩展容量。ly small initial capacity, which
     ...
     * {@code writerIndex} are {@code 0}.创建具有指定容量的新big-endian Java堆缓冲区，根据需要无限制地扩展其容量。新缓冲区的readerIndex和writerIndex是0。
     */
     ...
     * expands its capacity boundlessly on demand.  The new buffer's {@code readerIndex} and
     * {@code writerIndex} are {@code 0}.创建一个新的具有指定容量的big-endian直接缓冲区，根据需要无限扩展其容量。新缓冲区的readerIndex和writerIndex是0。
     */
     ...
     * {@code 0}.使用指定的初始容量创建一个新的big-endian Java堆缓冲区，该缓冲区可能会增长到maxCapacity，新缓冲区的readerIndex和writerIndex为0。
     */
     ...
     * {@code initialCapacity}, that may grow up to {@code maxCapacity}.
     * The new buffer's {@code readerIndex} and {@code writerIndex} are
     * {@code 0}.使用指定的初始容量创建一个新的big-endian直接缓冲区，该缓冲区可能会增长到maxCapacity。新缓冲区的readerIndex和writerIndex是0。
     */
     ...
     * returned buffer.创建一个封装指定数组的大endian缓冲区。对指定数组内容的修改将对返回的缓冲区可见。
     */
     ...
     * content will be visible to the returned buffer.创建一个新的big-endian缓冲区，该缓冲区封装指定数组的子区域。对指定数组内容的修改将对返回的缓冲区可见。
     */
     ...
     * 创建一个新的缓冲区，它封装指定的内存地址。如果doFree为真，那么一旦ByteBuf的引用计数达到0,memoryAddress将自动被释放。
     */
     ...
     * returned buffer.创建一个新的缓冲区，它包装指定缓冲区的可读字节。返回的缓冲区可以看到对指定缓冲区内容的修改。
     ...
     * content will be visible to the returned buffer.创建一个新的big-endian复合缓冲区，它封装指定的数组而不复制它们。返回的缓冲区可以看到对指定数组内容的修改。
     */
     ...
     * Creates a new big-endian composite buffer which wraps the readable bytes of the
     * specified buffers without copying them.  A modification on the content
     * of the specified buffers will be visible to the returned buffer.创建一个新的big-endian复合缓冲区，它封装指定缓冲区的可读字节，而不复制它们。返回的缓冲区可以看到对指定缓冲区内容的修改。
     ...
     * Creates a new big-endian composite buffer which wraps the slices of the specified
     * NIO buffers without copying them.  A modification on the content of the
     * specified buffers will be visible to the returned buffer.创建一个新的big-endian复合缓冲区，它封装指定NIO缓冲区的片，而不复制它们。返回的缓冲区可以看到对指定缓冲区内容的修改。
     */
    ...
    /**
     * Returns a new big-endian composite buffer with no components.返回一个没有组件的新big-endian复合缓冲区。
     */
     ...
     * {@code writerIndex} are {@code 0} and {@code array.length} respectively.创建一个新的big-endian缓冲区，其内容是指定数组的副本。新缓冲区的readerIndex和writerIndex是0和array。长度分别。
     */
     ...
     * the specified {@code length} respectively.创建一个新的big-endian缓冲区，其内容是指定数组的子区域的副本。新缓冲区的readerIndex和writerIndex分别为0和指定长度。
     */
     ...
     * respectively.创建一个新的缓冲区，其内容是指定缓冲区的当前片的副本。新缓冲区的readerIndex和writerIndex是0和buffer。剩下的分别。
     */
     ...
     * respectively.创建一个新的缓冲区，其内容是指定缓冲区的可读字节的副本。新缓冲区的readerIndex和writerIndex是0和buffer。readableBytes分别。
     */
     ...
     * {@code length} respectively.创建一个新的big-endian缓冲区，其内容是指定数组的合并副本。新缓冲区的readerIndex和writerIndex分别为0和所有数组的长度之和。
     */
     ...
     * {@code readableBytes} respectively.创建一个新的缓冲区，其内容是指定缓冲区的可读字节的合并副本。新缓冲区的readerIndex和writerIndex分别为0和所有缓冲区的readableBytes之和。
     *
     ...
     * {@code remaining} respectively.创建一个新的缓冲区，其内容是指定缓冲区片的合并副本。新缓冲区的readerIndex和writerIndex分别为0和所有缓冲区的剩余之和。
     *
     * @throws IllegalArgumentException
     *         if the specified buffers' endianness are different from each
     *         other
     ...
     * 创建一个新的big-endian缓冲区，其内容是指定字符集中编码的指定字符串。新缓冲区的readerIndex和writerIndex分别为0和编码字符串的长度。
     */
     ...
     * The new buffer's {@code readerIndex} and {@code writerIndex} are
     * {@code 0} and the length of the encoded string respectively.创建一个新的big-endian缓冲区，其内容是指定字符集中编码的指定数组。新缓冲区的readerIndex和writerIndex分别为0和编码字符串的长度。
     */
     ...
     * The new buffer's {@code readerIndex} and {@code writerIndex} are
     * {@code 0} and the length of the encoded string respectively.创建一个新的big-endian缓冲区，其内容是指定字符集中编码的指定数组的子区域。新缓冲区的readerIndex和writerIndex分别为0和编码字符串的长度。
     */
    ...
    /**
     * Creates a new 4-byte big-endian buffer that holds the specified 32-bit integer.创建一个新的4字节的big-endian缓冲区，该缓冲区保存指定的32位整数。
     */
    ...
    /**
     * Create a big-endian buffer that holds a sequence of the specified 32-bit integers.创建一个big-endian缓冲区，存放指定的32位整数序列。
     */
    ...
    /**
     * Creates a new 2-byte big-endian buffer that holds the specified 16-bit integer.创建一个新的2字节的big-endian缓冲区，该缓冲区保存指定的16位整数。
     */
    ...
    /**
     * Create a new big-endian buffer that holds a sequence of the specified 16-bit integers.创建一个新的big-endian buffer，该缓冲区保存指定的16位整数序列。
     */
    ...
    /**
     * Creates a new 3-byte big-endian buffer that holds the specified 24-bit integer.创建一个新的3字节的big-endian缓冲区，该缓冲区保存指定的24位整数。
     */
    ...
    /**
     * Create a new big-endian buffer that holds a sequence of the specified 24-bit integers.创建一个新的big-endian buffer，该缓冲区保存指定的24位整数序列。
     */
    ...
    /**
     * Return a unreleasable view on the given {@link ByteBuf} which will just ignore release and retain calls.对给定的ByteBuf返回一个不可发布的视图，它将忽略释放并保留调用。
     */
     ...
     * not try to slice the given {@link ByteBuf}s to reduce GC-Pressure.用不可修改的tebuf包装给定的ByteBufs。请注意，返回的ByteBuf不会尝试切割给定的ByteBuf以降低gc压力。
     */
