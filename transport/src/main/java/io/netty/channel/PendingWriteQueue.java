 ...
 * 等待以后执行的写操作队列。它还更新相关联的通道的可写性，以便考虑挂起的写操作来确定可写性。
 */
public final class PendingWriteQueue {
     ...
     *          删除所有挂起的写操作，并通过ChannelHandlerContext执行它们。写(对象,ChannelPromise)。
     */
     ...
     * 删除所有挂起的写操作，并使用给定的可抛出操作来失败它们。消息将通过referencecountuti . saferelease (Object)发布。
     */
     ...
     * {@link ReferenceCountUtil#safeRelease(Object)}.删除一个挂起的写操作，并使用给定的Throwable失败。消息将通过ReferenceCountUtil.safeRelease(Object)释放。
     */
     ...
     * {@link ChannelHandlerContext#write(Object, ChannelPromise)}.删除一个挂起的写操作，并通过ChannelHandlerContext执行它。写(对象,ChannelPromise)。
     *
     * @return  {@link ChannelFuture} if something was written and {@code null}
     *          if the {@link PendingWriteQueue} is empty.
     */
     ...
     * 删除一个挂起的写操作，并通过ReferenceCountUtil.safeRelease(Object)释放它的消息。
     *
     */
    ...
    /**
     * Holds all meta-data and construct the linked-list structure.保存所有元数据并构造链表结构。
     */
