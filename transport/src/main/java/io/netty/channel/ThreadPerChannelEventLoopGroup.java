...
/**
 * An {@link EventLoopGroup} that creates one {@link EventLoop} per {@link Channel}.为每个通道创建一个EventLoop的EventLoopGroup。
 */
public class ThreadPerChannelEventLoopGroup extends AbstractEventExecutorGroup implements EventLoopGroup {
    ...
    /**
     * Creates a new {@link EventLoop}.  The default implementation creates a new {@link ThreadPerChannelEventLoop}.创建一个新的EventLoop。默认实现创建一个新的ThreadPerChannelEventLoop。
     */
    ...
    public Future<?> shutdownGracefully(long quietPeriod, long timeout, TimeUnit unit) {
        ...
        // Notify the future if there was no children.如果没有孩子，就通知未来。
        if (isTerminated()) {
            terminationFuture.trySuccess(null);
        }
