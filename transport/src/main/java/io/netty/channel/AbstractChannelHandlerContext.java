...
//                自己实现的handler的渠道激活方法
    ...
    private void invokeChannelInactive() {
        if (invokeHandler()) {
            try {
//                最后会执行handler的方法，这个要开发来实现适配器实现的方法
...
//            如果任务取消了
     ...
     * 最好的方法是检测是否有ChannelHandler.handlerAdded(ChannelHandlerContext)被调用。如果不返回false，如果调用或无法检测返回true。如果这个方法返回false，我们将不会调用ChannelHandler，而只是转发事件。这是需要的，因为DefaultChannelPipeline可能已经将ChannelHandler放入了linkedlist中，而不是被称为ChannelHandler.handlerAdded(ChannelHandlerContext)。
     */
