 ...
 * </tr>
 * </table>
 * 除了ChannelConfig提供的选项外，SocketChannelConfig还允许选项映射中的以下选项:
 */
public interface SocketChannelConfig extends ChannelConfig {
     ...
     * {@code false}.StandardSocketOptions。TCP_NODELAY选项。请注意，与操作系统默认值(false)不同，此选项的默认值为true。然而，对于某些bug平台，如Android，在禁用Nagle算法的情况下显示不稳定的行为，默认值仍然为false。
     */
     ...
     * is {@code true} unlike the operating system default ({@code false}). However, for some buggy platforms, such as
     * Android, that shows erratic behavior with Nagle's algorithm disabled, the default value remains to be
     * {@code false}.设置StandardSocketOptions。TCP_NODELAY选项。请注意，与操作系统默认值(false)不同，此选项的默认值为true。然而，对于某些bug平台，如Android，在禁用Nagle算法的情况下显示不稳定的行为，默认值仍然为false。
     */
     ...
     * {@link Socket#setPerformancePreferences(int, int, int)}.设置在套接字中指定的性能参数。setPerformancePreferences(int,int,int)。
     */
     ...
     * is closed automatically when the remote peer shuts down output.返回true if且仅当该通道在其远程对等端关闭输出以使连接处于半关闭状态时不关闭自己。如果为false，则在远程对等点关闭输出时自动关闭连接。
     */
     ...
     * is closed automatically.设置当其远程对等端关闭输出以使连接处于半关闭状态时，通道是否不应关闭自己。如果为真，则在远程对等点关闭输出时不关闭连接。相反,ChannelInboundHandler。usereventtrigger (ChannelHandlerContext, Object)使用ChannelInputShutdownEvent对象进行调用。如果为false，连接将自动关闭。
     */
