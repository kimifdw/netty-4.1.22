...
/**
 * Base class for {@link Channel} implementations that are used in an embedded fashion.用于以嵌入式方式使用的通道实现的基类。
 */
public class EmbeddedChannel extends AbstractChannel {
    ...
    /**
     * Create a new instance with an {@link EmbeddedChannelId} and an empty pipeline.创建一个具有嵌入channelid和空管道的新实例。
     */
    ...
    /**
     * Create a new instance with the specified ID and an empty pipeline.使用指定的ID和空管道创建一个新实例。
     *
    ...
    /**
     * Create a new instance with the pipeline initialized with the specified handlers.使用指定的处理程序初始化的管道创建一个新实例。
     *
    ...
    /**
     * Register this {@code Channel} on its {@link EventLoop}.在它的EventLoop上注册这个通道。
     */
    ...
    /**
     * Returns the {@link Queue} which holds all the {@link Object}s that were received by this {@link Channel}.返回保存该通道接收到的所有对象的队列。
     */
    ...
    /**
     * Returns the {@link Queue} which holds all the {@link Object}s that were written by this {@link Channel}.返回保存该通道写入的所有对象的队列。
     */
     ...
     * method is conceptually equivalent to {@link #write(Object)}.将一条消息写入该通道的入站，而不刷新它。这个方法在概念上等价于写(对象)。
     *
    ...
    /**
     * Writes one message to the inbound of this {@link Channel} and does not flush it. This
     * method is conceptually equivalent to {@link #write(Object, ChannelPromise)}.将一条消息写入该通道的入站，而不刷新它。这个方法在概念上等同于写(对象，ChannelPromise)。
     *
    ...
    /**
     * Flushes the inbound of this {@link Channel}. This method is conceptually equivalent to {@link #flush()}.冲洗此通道的入站。这个方法在概念上等同于flush()。
     *
     * @see #flushOutbound()
     */
    ...
    /**
     * Mark this {@link Channel} as finished. Any further try to write data to it will fail.把这个频道标记为已完成。任何进一步尝试向它写入数据都将失败。
     *
     ...
     * Any further try to write data to it will fail.将此通道标记为已完成，并在入站和出站缓冲区中释放所有挂起的消息。任何进一步向其写入数据的尝试都将失败。
     *
     * @return bufferReadable returns {@code true} if any of the used buffers has something left to read
     */
     ...
     * for this {@link Channel} 在这个通道的EventLoop中运行所有正在等待的任务(也包括预定的任务)。
     */
     ...
     * {@code -1}.在该通道的EventLoop中运行所有挂起的计划任务，并在准备运行下一个计划任务时返回纳秒。如果没有调度其他任务，它将返回-1。
     */
     ...
     * an {@link Exception} if it isn't.如果通道是打开的，返回true;如果不是，则记录一个异常。
     */
    ...
    /**
     * Ensure the {@link Channel} is open and if not throw an exception.如果通道是打开的，返回true;如果不是，则记录一个异常。
     */
