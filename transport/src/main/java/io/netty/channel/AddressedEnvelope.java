 ...
 *           用发件人地址和收件人地址包装另一条消息的消息。
 */
public interface AddressedEnvelope<M, A extends SocketAddress> extends ReferenceCounted {
    /**
     * Returns the message wrapped by this envelope message.返回由此信封消息包装的消息。
     */
    M content();

    /**
     * Returns the address of the sender of this message.返回此消息的发送方的地址。
     */
    A sender();

    /**
     * Returns the address of the recipient of this message.返回此消息接收者的地址。
     */
    A recipient();
