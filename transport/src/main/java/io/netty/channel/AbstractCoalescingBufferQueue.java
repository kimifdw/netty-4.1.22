...
public abstract class AbstractCoalescingBufferQueue {
     ...
     * all the buffer's bytes have been consumed from the queue and written.将缓冲区添加到队列的前面，并将当所有缓冲区的字节从队列中消耗并写入时应该完成的承诺与它关联。
     ...
     * all the buffer's bytes have been consumed from the queue and written.在队列的末尾添加一个缓冲区，并将当所有缓冲区的字节从队列中消耗并写入时应该完成的承诺与之关联。
     ...
     *                 将缓冲区添加到队列的末尾，并将一个侦听器与它关联，当所有缓冲区字节从队列中消耗并写入时，该侦听器应该被完成。
     */
     ...
     * 用指定的字节数从队列中删除ByteBuf。在删除过程中，任何添加的缓冲区都将被完全消耗，当传递的聚合通道承诺完成时，它的承诺就会完成。
     */
    ...
    /**
     *  Release all buffers in the queue and complete all listeners and promises.释放队列中的所有缓冲区并完成所有侦听器和承诺。
     */
    ...
    /**
     * Copy all pending entries in this queue into the destination queue.将此队列中的所有挂起项复制到目标队列中。
    ...
    /**
     * Calculate the result of {@code current + next}.计算当前+的结果
     */
     ...
     * {@link #compose(ByteBufAllocator, ByteBuf, ByteBuf)}.计算第一个ByteBuf，它将被用于随后的调用（ByteBufAllocator，ByteBuf，ByteBuf）。
     */
     ...
     * @return the {@link ByteBuf} which represents an empty queue.当移除（ByteBufAllocator，int，ChannelPromise）时返回的值被调用，但是队列是空的。
     */
     ...
     * @return the number of elements in this queue.通过一个add(ByteBuf)方法获得该队列中添加的元素的数量。
     */
