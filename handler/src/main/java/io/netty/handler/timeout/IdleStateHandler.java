 ...
 * 当通道暂时没有执行读、写或同时执行两个操作时，触发IdleStateEvent。
支持空闲状态
财产
意义
readerIdleTime
一个状态为IdleState的IdleStateEvent。当指定时间段内没有执行读操作时，将触发READER_IDLE。指定0禁用。
writerIdleTime
州为州的闲散状态。当指定时间段内没有执行写操作时，将触发WRITER_IDLE。指定0禁用。
allIdleTime
州为州的闲散状态。当在指定的时间段内不执行读写操作时，将触发ALL_IDLE。指定0禁用。
 */
public class IdleStateHandler extends ChannelDuplexHandler {
     ...
     * 当要触发IdleStateEvent时调用。这个实现调用ChannelHandlerContext.fireUserEventTriggered(对象)。
     */
