 ...
 * </p>
 *
 计算读和写字节的数量，以获得限制速率的通信量。
 它在给定的检查间隔定期计算入站和出站流量的统计数据，并调用AbstractTrafficShapingHandler.doAccounting(traffic counter)方法。如果校验间隔为0，则不会进行会计操作，统计数据只能在每个接收或写入操作中计算。
 */
public class TrafficCounter {
    ...
    /**
     * Last writing time during current check interval 当前检查间隔内的最后写入时间
     */
    private long writingTime;

    /**
     * Last reading delay during current check interval 当前检查间隔期间的上一次读取延迟
     */
    ...
    /**
     * Last Time where cumulative bytes where reset to zero: this time is a real EPOC time (informative only) 上次，累计字节在哪里重置为零:这次是一个真实的EPOC时间(仅提供信息)
     */
    ...
    /**
     * Last written bytes number during last check interval 上次检查间隔期间最后写入的字节数
     */
    private volatile long lastWrittenBytes;

    /**
     * Last read bytes number during last check interval 上次检查间隔期间的最后读取字节数
     */
    private volatile long lastReadBytes;

    /**
     * Last future writing time during last check interval 最后一次写入时间在最后一次检查间隔
     */
    private volatile long lastWritingTime;

    /**
     * Last reading time during last check interval 最后一次阅读时间在最后一次检查间隔
     */
    ...
    /**
     * The associated TrafficShapingHandler 相关TrafficShapingHandler
     */
    final AbstractTrafficShapingHandler trafficShapingHandler;

    /**
     * Executor that will run the monitor 将运行监视器的执行程序
     */
    ...
    /**
     * Class to implement monitoring at fix delay 类，用于在修复延迟时实现监视
     *
     */
    ...
    /**
     * Start the monitoring process.开始监控过程。
     */
    ...
    /**
     * Stop the monitoring process.停止监控过程。
     */
    ...
    /**
     * Reset the accounting on Read and Write.重新设定读和写的会计。
     *
     ...
     *            它的构造函数有宿主它的AbstractTrafficShapingHandler、要使用的ScheduledExecutorService、其名称、两个计算之间的校验间隔(毫秒)。
     */
     ...
     *            the checkInterval in millisecond between two computations.
     *
    构造函数，包含托管它的AbstractTrafficShapingHandler、要使用的计时器、它的名称、两个计算之间的校验间隔(毫秒)。
     */
    ...
    /**
     * Change checkInterval between two computations in millisecond.以毫秒为单位更改两个计算之间的校验间隔。
     *
    ...
    /**
     * Computes counters for Read.计算计数器的阅读。
     *
     * @param recv
    ...
    /**
     * Computes counters for Write.为编写计算计数器。
     *
     * @param write
    ...
    /**
     * Computes counters for Real Write.计算实际写入的计数器。
     *
     * @param write
     *            the size in bytes to write
     */
     ...
     * 重置从System.currentTimeMillis()中读取和写入的累计字节计数器和相关的绝对时间。
     */
     ...
     * time.返回等待给定长度消息的时间(如果有的话)，使用给定的有限流量和最大等待时间。
     *
     * @param size
     *            the recv size
     * @param limitTraffic
     ...
     * the max wait time.使用给定的限制通信量和最大等待时间，返回等待给定长度消息的时间(如果有的话)。
     *
     * @param size
     *            the write size
     * @param limitTraffic
     ...
     * @param now the current time
     * @return the current time to wait (in ms) if needed for Write operation.
     * 返回等待给定长度消息的时间(如果有的话)，使用给定的有限流量和最大等待时间。
     */
