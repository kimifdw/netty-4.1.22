 ...
 *
 AbstractTrafficShapingHandler的这个实现是针对全局和每个通道的流量整形，也就是说带宽的全局限制，无论打开的通道的数量和每个通道带宽的限制。本版本不应与其他版本的车辆在同一管道中。
 一般用途应如下:
 创建您独特的GlobalChannelTrafficShapingHandler，如:GlobalChannelTrafficShapingHandler myHandler =新的GlobalChannelTrafficShapingHandler(执行者);执行器可以是底层IO worker池管道。addlast (myHandler);注意，该处理程序具有“all”的管道覆盖，这意味着必须在所有通道之间创建和共享此类处理程序，因为计数器必须在所有通道之间共享。其他参数可以传递，如写或读限制(以字节/秒为单位，0表示没有限制)或检查间隔(以毫秒为单位)，它表示两个带宽计算之间的延迟，因此doAccounting方法的回调(0表示根本没有记帐)。请注意，由于这是全局和通道通信量整形的融合，限制在两个集合中，分别是全局和通道。
 0的值表示不考虑checkInterval。如果您需要进行流量整形，但不需要这样的计算，那么建议设置一个正值，即使这个值很高，因为流量整形的精度取决于计算流量的周期。区间越高，对交通的影响就越小。它被认为是高价值的东西接近5或10分钟。maxTimeToWait默认设置为15秒，允许指定时间形成的上界。
 ...
 一些配置方法将被认为是最好的努力，这意味着所有已经计划的交通将不会被改变，而仅仅应用于新的交通。因此，对这些方法的期望使用不会太频繁，因此对流量形成配置也会有影响。
...
public class GlobalChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
    ...
    /**
     * All queues per channel 每个频道所有队列
     */
    final ConcurrentMap<Integer, PerChannel> channelQueues = PlatformDependent.newConcurrentHashMap();

    /**
     * Global queues size 全球队列大小
     */
    private final AtomicLong queuesSize = new AtomicLong();

    /**
     * Maximum cumulative writing bytes for one channel among all (as long as channels stay the same) 在所有信道中(只要信道保持不变)为一个信道的最大累积写入字节
     */
    private final AtomicLong cumulativeWrittenBytes = new AtomicLong();

    /**
     * Maximum cumulative read bytes for one channel among all (as long as channels stay the same) 在所有信道中(只要信道保持不变)为一个信道的最大累积读字节
     */
    ...
    /**
     * Create the global TrafficCounter 创建全球交通
     */
