 ...
 * SslContext的实现，它与支持OpenSsl C库API的库一起工作。
 必须释放该类的实例，否则本机内存将泄漏!
 在释放依赖于该类实例的任何ReferenceCountedOpenSslEngine之前，不能释放该类的实例。否则，如果调用使用该类的JNI资源的ReferenceCountedOpenSslEngine方法，JVM可能会崩溃。
 */
public abstract class ReferenceCountedOpenSslContext extends SslContext implements ReferenceCounted {
     ...
     * OpenSSL SSL_CTX对象。在使用ctx时，必须保持ctxLock !
     */
    protected long ctx;
     ...
     * At this point {@code 0} will be returned.返回指向此ReferenceCountedOpenSslContext的SSL_CTX对象的指针。请注意，一旦finalize()方法被调用，它就会被释放。此时将返回0。
     *
     ...
     * to initiate a renegotiation a {@link SSLHandshakeException} will be thrown during decoding.说明是否支持远程启动的重新协商。如果不支持，远程方尝试启动重新谈判，将在解码过程中抛出SSLHandshakeException。
     */
    @Deprecated
     ...
     * (e.g. handshake, renegotiation, etc...).设置BIO用于基于非应用程序的写(例如握手、重新协商等)的缓冲区的大小。
     */
    ...
    /**
     * Returns the size of the buffer used by the BIO for non-application based writes 返回BIO用于基于非应用程序的写操作的缓冲区的大小
     */
    ...
    /**
     * Sets the SSL session ticket keys of this context.设置此上下文的SSL会话票证键。
     *
     ...
     * At this point {@code 0} will be returned.返回指向此ReferenceCountedOpenSslContext的SSL_CTX对象的指针。请注意，只要调用了release()方法，就会释放它。此时将返回0。
     *
     * @deprecated this method is considered unsafe as the returned pointer may be released later. Dont use it!
     */
    @Deprecated
     ...
     * {@link OpenSslApplicationProtocolNegotiator} object.将ApplicationProtocolConfig对象转换为opensslationprotocol协商器对象。
     *
     ...
     * or {@code 0} if the {@code key} is {@code null}. The BIO contains the content of the {@code key}.如果键为空，返回指向内存中的BIO或0的指针。BIO包含密钥的内容。
     */
    ...
    /**
     * Return the pointer to a <a href="https://www.openssl.org/docs/crypto/BIO_get_mem_ptr.html">in-memory BIO</a>
     * or {@code 0} if the {@code certChain} is {@code null}. The BIO contains the content of the {@code certChain}.如果certChain为空，则返回指向内存中的BIO或0的指针。BIO包含certChain的内容。
     */
