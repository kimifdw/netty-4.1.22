 ...
 * ...
 * </pre>
 * 作为SSLEngine和SslHandler工厂的安全套接字协议实现。在内部，它是通过JDK的SSLContext或OpenSSL的SSL_CTX实现的。
 使您的服务器支持SSL/TLS
 使您的客户端支持SSL/TLS
 */
public abstract class SslContext {
    ...
    /**
     * Returns the default server-side implementation provider currently in use.返回当前使用的默认服务器端实现提供程序。
     *
    ...
    /**
     * Returns the default client-side implementation provider currently in use.返回当前使用的默认客户端实现提供程序。
     *
     * @return {@link SslProvider#OPENSSL} if OpenSSL is available. {@link SslProvider#JDK} otherwise.
     */
    ...
    /**
     * Creates a new instance (startTls set to {@code false}).创建一个新实例(startTls设置为false)。
     */
    ...
    /**
     * Returns {@code true} if and only if this context is for server-side.如果且仅当此上下文用于服务器端时返回true。
     */
    ...
    /**
     * Returns the {@code true} if and only if this context is for client-side.如果且仅当此上下文用于客户端，则返回true。
     */
    public abstract boolean isClient();

    /**
     * Returns the list of enabled cipher suites, in the order of preference.按优先顺序返回启用的密码套件列表。
     */
    public abstract List<String> cipherSuites();

    /**
     * Returns the size of the cache used for storing SSL session objects.返回用于存储SSL会话对象的缓存大小。
     */
    public abstract long sessionCacheSize();

    /**
     * Returns the timeout for the cached SSL session objects, in seconds.以秒为单位返回缓存的SSL会话对象的超时。
     */
    ...
    /**
     * Returns the object responsible for negotiating application layer protocols for the TLS NPN/ALPN extensions.返回负责为TLS NPN/ALPN扩展协商应用层协议的对象。
     */
     ...
     * 创建一个新的SSLEngine。
    如果SslProvider。使用OPENSSL_REFCNT后，对象必须被释放。一种方法是将SslHandler包装起来，并将其插入到管道中。看到newHandler(ByteBufAllocator)。
     */
     ...
     *
     * @return a new {@link SSLEngine}
     * 使用咨询同行信息创建一个新的SSLEngine。
    如果SslProvider。使用OPENSSL_REFCNT后，对象必须被释放。一种方法是将SslHandler包装起来，并将其插入到管道中。看到newHandler(ByteBufAllocator,字符串,int)。
     */
    public abstract SSLEngine newEngine(ByteBufAllocator alloc, String peerHost, int peerPort);

    /**
     * Returns the {@link SSLSessionContext} object held by this context.返回此上下文持有的SSLSessionContext对象。
     */
     ...
     * 创建一个新的SslHandler。
    如果SslProvider。然后返回的SslHandler将释放被包装的引擎。如果返回的SslHandler没有插入到管道中，那么您可能会泄漏本机内存!
    注意:底层生成的SSLEngine默认情况下不会启用主机名验证。如果您为客户端创建SslHandler并希望获得适当的安全性，我们建议您配置SSLEngine(请参阅javax.net. sslparameter . setendpointidentificationalgorithm (String)):
    底层的SSLEngine可能不会遵循SSLEngine javadocs所施加的限制，它限制了对单个SSL/TLS包的操作。
     */
     ...
     * @return a new {@link SslHandler}
     *
    创建一个具有咨询同行信息的新SslHandler。
    如果SslProvider。然后返回的SslHandler将释放被包装的引擎。如果返回的SslHandler没有插入到管道中，那么您可能会泄漏本机内存!
    注意:底层生成的SSLEngine默认情况下不会启用主机名验证。如果您为客户端创建SslHandler并希望获得适当的安全性，我们建议您配置SSLEngine(请参阅javax.net. sslparameter . setendpointidentificationalgorithm (String)):
    底层SSLEngine可能不遵循SSLEngine javadocs所施加的限制，它限制wrap/unwrap对单个SSL/TLS数据包进行操作。
     */
    ...
    /**
     * Build a {@link TrustManagerFactory} from a certificate chain file.从证书链文件构建信任管理器工厂。
