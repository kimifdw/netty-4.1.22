 ...
 * @see #valueOf(byte[])
 * @see #valueOf(ByteBuf)
 * 这是一个PrivateKey的特殊目的实现，它允许用户将PEM/PKCS#8编码的关键材料直接传递到OpenSslContext中，而无需在Java中解析和重新编码字节。除了在PemEncoded和驱逐舰中实现的所有方法都是不支持操作异常。
 */
public final class PemPrivateKey extends AbstractReferenceCounted implements PrivateKey, PemEncoded {
     ...
     * No input validation is performed to validate it.从原始字节[]创建PemPrivateKey。注意:假定给定的参数是PEM/PKCS#8编码的值。没有执行输入验证来验证它。
     */
     ...
     *
     * ATTENTION: It's assumed that the given argument is a PEM/PKCS#8 encoded value.
     * No input validation is performed to validate it.从原始的ByteBuf创建一个PemPrivateKey。注意:假定给定的参数是PEM/PKCS#8编码的值。没有执行输入验证来验证它。
     */
     ...
     * this method.注意:这是一个JDK8接口/方法。由于向后兼容性的原因，不可能将@Override注释附加到这个方法上。
     *
