 ...
 * 使用OpenSSL生物抽象实现SSLEngine。
 必须释放该类的实例，否则本机内存将泄漏!
 在释放实例所依赖的ReferenceCountedOpenSslContext之前，必须释放该类的实例。否则，如果调用该类的任何方法使用JVM可能崩溃的ReferenceCountedOpenSslContext JNI资源的话。
 */
public class ReferenceCountedOpenSslEngine extends SSLEngine implements ReferenceCounted, ApplicationProtocolAccessor {
     ...
     * At this point {@code 0} will be returned.返回指向此referencountedopensslengine的SSL对象的指针。请注意，只要调用了release()或shutdown()方法，它就会被释放。此时将返回0。
     */
    ...
    /**
     * Read plaintext data from the OpenSSL internal BIO 从OpenSSL内部BIO读取明文数据
     */
     ...
     * 这种方法是故意不同步的，只有在知道您在EventLoop线程中才会使用，而max包装开销的可见性是通过其他同步块实现的。
     */
     ...
     * 这种方法是故意不同步的，只有当您知道您在EventLoop线程中，并且在max包装缓冲区大小和max包装开销上的可见性是通过其他同步块实现的。
     */
    ...
    /**
     * It is assumed this method is called in a synchronized block (or the constructor)! 假定在同步块(或构造函数)中调用此方法!
     */
     ...
     * TLS不支持从客户的角度为非连续版本做广告，客户端只宣传最大支持版本。TLS协议也不支持离散协议的所有不同组合，而是假设连续的范围。如果使用非连续协议(即使有一组兼容的协议和密码)，OpenSSL也会有一些意外的行为(例如握手失败)。由于这些原因，该方法将确定OpenSSL中的最小协议和最大协议，并启用从[min协议，max协议]到的连续范围。
     */
    @Override
    ...
    /**
     * Converts the specified OpenSSL cipher suite to the Java cipher suite.将指定的OpenSSL密码套件转换为Java密码套件。
     */
    ...
    /**
     * Converts the protocol version string returned by {@link SSL#getVersion(long)} to protocol family string.将SSL.getVersion(long)返回的协议版本字符串转换为协议系列字符串。
     */
    ...
    private final class OpenSslSession implements SSLSession {
         ...
         * and {@link #getPeerCertificates()}.可以通过getPeerCertificateChain()和getPeerCertificates()获得的Init对等证书。
         */
         ...
         *                             扩展(或增加)getApplicationBufferSize()返回的值。
        这只在同步块中调用，因此不需要使用原子操作。
         */
