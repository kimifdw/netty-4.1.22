...
public abstract class SimpleTrustManagerFactory extends TrustManagerFactory {
     ...
     * SimpleTrustManagerFactory。SimpleTrustManagerFactorySpi必须有一个指向SimpleTrustManagerFactory的引用，才能将其回调委托给simpletrustmanagementerfactory。但是，这样做是不可能的，因为TrustManagerFactory在构建时需要TrustManagerFactorySpi，并且不提供以后访问它的方法。为了解决这个问题，我们使用了一个使用ThreadLocal的丑陋的hack。
     */
    ...
    /**
     * Initializes this factory with a source of certificate authorities and related trust material.使用证书颁发机构和相关信任材料的来源初始化这个工厂。
     *
    ...
    /**
     * Initializes this factory with a source of provider-specific key material.使用证书颁发机构和相关信任材料的来源初始化这个工厂。
     *
    ...
    /**
     * Returns one trust manager for each type of trust material.为每种类型的信任材料返回一个信任管理器。
     *
