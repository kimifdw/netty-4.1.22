...
/**
 * Converts a Java cipher suite string to an OpenSSL cipher suite string and vice versa.将Java密码套件字符串转换为OpenSSL密码套件字符串，反之亦然。
 *
...
final class CipherSuiteConverter {
     ...
     * Note that the Java cipher suite has the protocol prefix (TLS_, SSL_) 注意，Java密码套件具有协议前缀(TLS_， SSL_)
     */
     ...
     * a Java cipher suite has the protocol name prefix (TLS_, SSL_) openssl - java密码套件转换映射。注意，一个OpenSSL密码套件可以转换为多个Java密码套件，因为Java密码套件具有协议名称前缀(TLS_， SSL_)
     */
    ...
    /**
     * Tests if the specified key-value pair has been cached in Java-to-OpenSSL cache.测试指定的键值对是否缓存在Java-to-OpenSSL缓存中。
     */
    ...
    /**
     * Tests if the specified key-value pair has been cached in OpenSSL-to-Java cache.测试指定的键值对是否缓存在OpenSSL-to-Java缓存中。
     */
    ...
    /**
     * Converts the specified Java cipher suites to the colon-separated OpenSSL cipher suite specification.将指定的Java密码套件转换为冒号分隔的OpenSSL密码套件规范。
     */
    ...
    /**
     * Converts the specified Java cipher suite to its corresponding OpenSSL cipher suite name.将指定的Java密码套件转换为其对应的OpenSSL密码套件名称。
     *
    ...
    /**
     * Convert from OpenSSL cipher suite name convention to java cipher suite name convention.将OpenSSL密码套件名称约定转换为java密码套件名称约定。
