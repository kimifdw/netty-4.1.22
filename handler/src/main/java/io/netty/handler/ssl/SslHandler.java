 ...
 * 向通道添加SSL·TLS和StartTLS支持。请参阅发行版或网站中的“SecureChat”示例以了解详细用法。
 开始握手
 除了使用ChannelFuture来获得关于完成握手的通知之外，还可以通过实现ChannelInboundHandler来检测它。usereventtrigger (ChannelHandlerContext, Object)方法并检查SslHandshakeCompletionEvent。
 握手
 当通道处于活动状态并且SSLEngine.getUseClientMode()返回true时，将自动为您发出握手。所以没有必要自己去纠结。
 关闭会话
 要关闭SSL会话，应该调用close()方法将close_notify消息发送到远程对等点。一个例外是当您关闭通道时——SslHandler拦截关闭请求并在通道关闭之前自动发送close_notify消息。一旦SSL会话被关闭，它就不是可重用的，因此您应该使用一个新的SSLEngine创建一个新的SslHandler，如下面的部分所述。
 重新启动会话
 要重新启动SSL会话，必须从ChannelPipeline中删除现有的封闭SslHandler，将一个新的SslHandler插入到管道中，并按照第一部分的描述启动握手过程。
 实现StartTLS
 StartTLS是一种通信模式，用于保护明文连接中间的连接。请注意，它与SSL·TLS不同，它从连接的开始处获得了连接。通常，StartTLS由三个步骤组成:
 客户端向服务器发送一个StartTLS请求。
 服务器向客户端发送一个StartTLS响应。
 客户开始SSL握手。
 如果您实现了一个服务器，您需要:
 创建一个新的SslHandler实例，并将startTls标志设置为true，
 将SslHandler插入到ChannelPipeline，并
 写一个StartTLS响应。
 请注意，在发送StartTLS响应之前必须插入SslHandler。否则，客户端可以在SslHandler插入到ChannelPipeline之前发送begin SSL握手，从而导致数据损坏。
 客户端实现要简单得多。
 写一个StartTLS请求,
 等待StartTLS的回应，
 创建一个新的SslHandler实例，将startTls标志设置为false，
 将SslHandler插入到ChannelPipeline，并
 启动SSL握手。
 已知的问题
 由于Java附带的SslEngine当前实现的一个已知问题，您可能会在完成完整的GC时看到阻塞的io - thread。
 因此，如果你受到影响，你可以通过调整缓存设置来解决这个问题，如下所示:
 这里要使用的值取决于应用程序的性质，应该基于对应用程序的监视和调试来设置。有关更多细节，请参阅问题跟踪器中的#832。
 */
public class SslHandler extends ByteToMessageDecoder implements ChannelOutboundHandler {
     ...
     * 在unwrapNonAppData(ChannelHandlerContext)中用作unwrap(ChannelHandlerContext, ByteBuf, int, int)的输入。使用这个静态实例将对象创建减少为#nioBuffer()，每次都创建一个新的ByteBuffer。
     */
     ...
     * allowed by the TLS RFC.2 ^ 14所允许的最大尺寸的明文块TLS RFC。
     */
    ...
    private enum SslEngineType {
        ...
        /**
         * {@code true} if and only if {@link SSLEngine} expects a direct buffer.如果且仅当SSLEngine希望有一个直接缓冲区时，则是正确的
         */
         ...
         * 当使用JDK SSLEngine时，我们使用mergecum调制器，因为它只适用于一个ByteBuffer。当使用OpenSslEngine时，我们可以使用复合程序，因为它有OpenSslEngine。unwrap（ByteBuffer，ByteBuffer），它与多个ByteBuffer一起工作，并且不需要做额外的内存拷贝。
         */
     ...
     * creation.如果SSLEngine使用。包装(ByteBuffer[],ByteBuffer)和SSLEngine。unwrap（ByteBuffer，ByteBuffer）应该用ByteBuf来调用，该ByteBuf只支持一个ByteBuffer来减少对象的创建。
     */
     ...
     * 当产生某些东西时，由wrap（）方法设置。channelReadComplete（ChannelHandlerContext）将检查这个标志，清除它，并调用ctx.冲洗（）
     */
     ...
     * when {@link ChannelConfig#isAutoRead()} is {@code false}.这个标志用于确定我们是否需要调用ChannelHandlerContext.read（）来在channelconfig.isautoread（）为false时消耗更多的数据。
     */
     ...
     *                     设置要传递给每个SSLEngine的字节数。包装（ByteBuffer、int、int、ByteBuffer）调用。

    如果包装的<=0，那么我们将把每个数据块都写为is。
    如果汇总数据大小，我们将尝试聚合多个数据块。
    如果包装上的数据大小，如果包装的<=数据大小，那么我们将在编写时将数据分成包块。
    如果SSLEngine不支持采集包装操作（例如SslProvider.OPENSSL），那么在包装之前聚合数据可以帮助减少TLS开销与数据负载之间的比率，这将导致更好的goodput。编写固定的数据块也可以帮助定位底层传输的（如TCP）框架大小。在lossy/拥塞的网络条件下，这可能会帮助对等点更早地获得完整的TLS数据包，并且能够更早地完成工作，而不是等待TLS数据包的所有部分到达。
     */
    @UnstableApi
     ...
     * forcibly.获取关闭通道触发的关闭通知的超时时间。如果在给定的超时中关闭了关闭通知，通道将被强制关闭
     */
     ...
     * {@link Channel}. If the close_notify was not flushed in the given timeout the {@link Channel} will be closed
     * forcibly.设置关闭通道触发关闭通知的超时时间。如果在给定的超时中关闭了关闭通知，通道将被强制关闭。
     */
     ...
     * 获得超时（in ms），以接收关闭通道触发的关闭通知的响应。这个超时开始于关闭通知消息成功地写入远程对等点之后。使用0直接关闭通道，而不是等待响应。
     */
     ...
     * {@link Channel}. This timeout starts after the close_notify message was successfully written to the
     * remote peer. Use {@code 0} to directly close the {@link Channel} and not wait for the response.
     * 设置超时，以接收关闭通道触发的关闭通知的响应。这个超时开始于关闭通知消息成功地写入远程对等点之后。使用0直接关闭通道，而不是等待响应。
     */
    ...
    /**
     * Returns the {@link SSLEngine} which is used by this handler.返回此处理程序使用的SSLEngine。
     */
    ...
    /**
     * Returns the name of the current application-level protocol.返回当前应用程序级协议的名称。
     *
    ...
    /**
     * Returns a {@link Future} that will get notified once the current TLS handshake completes.返回一个将在当前TLS握手完成后得到通知的未来。
     *
     ...
     * destroys the underlying {@link SSLEngine}.向指定的通道发送SSL关闭通知消息，并销毁底层的SSLEngine。
     *
     ...
     * 检查给定的可抛出项是否可以忽略，并在关闭ssl连接时“吞掉”，发送close_notify消息。但是，在对等点也发送close_notify之后，并不一定要接收close_notify。发送初始close_notify的一方可以立即关闭连接，然后对等方将获得连接重置错误。
     *
     */
     ...
     *                  如果给定的ByteBuf被加密，则返回true。注意，此方法不会增加给定ByteBuf的readerIndex。
     */
    ...
    /**
     * Calls {@link SSLEngine#unwrap(ByteBuffer, ByteBuffer)} with an empty buffer to handle handshakes, etc.调用SSLEngine。用空缓冲区打开(ByteBuffer、ByteBuffer)来处理握手等。
     */
     ...
     * 从SSLEngine获取所有委托任务，并通过委托任务执行器运行它们。如果委托的taskexecutor是immediate型executor，只需直接调用Runnable.run()而不是使用Executor.execute(Runnable)。否则，通过委派的taskexecutor运行任务，并等待任务完成。
     */
     ...
     * go straight into {@link HandshakeStatus#NOT_HANDSHAKING} when handshake is finished.在一些Android SSLEngine实现上工作，跳过SSLEngineResult.HandshakeStatus。完成后直接进入sslengineresulesr . handshakestatus。握手结束时不要握手。
     *
    ...
    /**
     * Notify all the handshake futures about the successfully handshake 通知所有的握手未来成功的握手
     */
     ...
     * the specified amount of pending bytes.为SSLEngine分配出站网络缓冲区。wrap（ByteBuffer，ByteBuffer），它可以加密指定的待定字节数。
     */
     ...
     * 每个对sslwrite的调用都会引入大约100个字节的开销。这个合并队列试图通过将明文聚合在一起来增加goodput。如果写了许多小块，可以增加goodput，减少对sslwrite的调用量，并减少总体加密操作。
     */
