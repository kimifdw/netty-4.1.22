...
public interface JdkApplicationProtocolNegotiator extends ApplicationProtocolNegotiator {
    /**
     * Abstract factory pattern for wrapping an {@link SSLEngine} object. This is useful for NPN/APLN JDK support.用于包装SSLEngine对象的抽象工厂模式。这对于NPN/APLN JDK支持很有用。
     */
     ...
     * handshake.接口来定义应用程序协议选择器在SSL握手过程中的角色。每次SSL握手都会调用unsupported()或select(List)。
     */
     ...
     * will be called for each SSL handshake.侦听器被它的对等方选择的协议通知。对于每个SSL握手，都将调用不支持的()或选择的(String)方法。
     */
