...
/**
 * Provides a means to filter the supplied cipher suite based upon the supported and default cipher suites.提供一种基于支持和默认密码套件过滤所提供密码套件的方法。
 */
public interface CipherSuiteFilter {
    /**
     * Filter the requested {@code ciphers} based upon other cipher characteristics.根据其他密码特性过滤所请求的密码。
