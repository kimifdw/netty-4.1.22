 ...
 * @see #valueOf(byte[])
 * @see #valueOf(ByteBuf)
 * 这是X509Certificate的特殊目的实现，它允许用户将PEM/PKCS#8编码的数据直接传递到OpenSslContext中，而无需在Java中解析和重新编码字节。除了在PemEncoded不支持操作异常中实现的所有方法。
 */
public final class PemX509Certificate extends X509Certificate implements PemEncoded {
    ...
    /**
     * Creates a {@link PemEncoded} value from the {@link X509Certificate}s.从x509证书创建一个PemEncoded值。
     */
     ...
     * If the {@link ByteBuf} didn't exist yet it'll create it using the {@link ByteBufAllocator}.将PemEncoded值追加到ByteBuf(最后一个arg)并返回。如果ByteBuf还不存在，它将使用ByteBufAllocator创建它。
     */
     ...
     * If the {@link ByteBuf} didn't exist yet it'll create it using the {@link ByteBufAllocator}.将x509证书值附加到ByteBuf(最后一个arg)并返回它。如果ByteBuf还不存在，它将使用ByteBufAllocator创建它。
     */
    private static ByteBuf append(ByteBufAllocator allocator, boolean useDirect,
     ...
     * No input validation is performed to validate it.从原始字节创建一个pemx509证书[]。注意:假定给定的参数是PEM/PKCS#8编码的值。没有执行输入验证来验证它。
     */
     ...
     *
     * ATTENTION: It's assumed that the given argument is a PEM/PKCS#8 encoded value.
     * No input validation is performed to validate it.从生的ByteBuf创建一个pemx509证书。注意:假定给定的参数是PEM/PKCS#8编码的值。没有执行输入验证来验证它。
     */
