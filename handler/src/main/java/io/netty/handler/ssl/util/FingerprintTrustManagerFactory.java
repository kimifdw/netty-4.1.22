 ...
 * </pre>
 * </p>
 * 信任一个信任X.509证书的信任管理工厂，该证书的SHA1校验和匹配。
 注意:建议验证证书及其链，以防止中间人攻击。这个TrustManagerFactory只会验证证书的指纹是否与给定的指纹之一匹配。这个过程称为证书固定，是一种有效的保护。为了获得最大的安全性，应该验证整个证书链是否符合预期。值得一提的是，在企业环境中发现的某些防火墙、代理或其他设备实际上执行中间人攻击，从而呈现不同的证书指纹。
 X.509证书的SHA1校验和是从它的DER编码格式计算出来的。您可以使用openssl命令获取X.509证书的指纹。例如:
 */
