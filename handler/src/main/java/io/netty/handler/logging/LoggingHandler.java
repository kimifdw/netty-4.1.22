 ...
 * By default, all events are logged at <tt>DEBUG</tt> level.使用日志框架记录所有事件的ChannelHandler。默认情况下，所有事件都记录在调试级别。
 */
@Sharable
...
public class LoggingHandler extends ChannelDuplexHandler {
     ...
     * name of the instance with hex dump enabled.创建一个新的实例，其logger名称是启用了十六进制转储的实例的完全限定类名。
     */
     ...
     * enabled.使用指定的日志记录器名称和启用十六进制转储创建一个新实例。
     *
    ...
    /**
     * Creates a new instance with the specified logger name using the default log level.使用默认日志级别创建具有指定日志记录器名称的新实例。
     *
    ...
    /**
     * Formats an event and returns the formatted message.格式化事件并返回格式化消息。
     *
    ...
    /**
     * Generates the default log message of the specified event whose argument is a {@link ByteBuf}.生成指定事件的默认日志消息，该事件的参数是ByteBuf。
     */
    ...
    /**
     * Generates the default log message of the specified event whose argument is a {@link ByteBufHolder}.生成指定事件的默认日志消息，其参数为ByteBufHolder。
     */
    ...
    /**
     * Generates the default log message of the specified event whose argument is an arbitrary object.生成参数为任意对象的指定事件的默认日志消息。
     */
