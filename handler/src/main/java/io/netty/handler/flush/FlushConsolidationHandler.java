 ...
 * 用于合并Channel.flush() / ChannelHandlerContext.flush()操作(也包括Channel.writeAndFlush(Object) / Channel)的ChannelDuplexHandler。writeAndFlush(对象，ChannelPromise)和ChannelOutboundInvoker.writeAndFlush(对象)/ ChannelOutboundInvoker。writeAndFlush(对象,ChannelPromise))。
 冲洗操作通常比较昂贵，因为这些操作可能在传输层触发一个syscall。因此，在大多数情况下(在这种情况下，写延迟可以与吞吐量交换)，最好尽量减少刷新操作。
 如果一个读循环当前正在进行，那么刷新(ChannelHandlerContext)将不会被传递到channel管道中的下一个ChannelOutboundHandler，因为当channelReadComplete(ChannelHandlerContext)被触发时，它将接收所有未处理的刷新。如果没有读循环正在进行，那么行为取决于consolidated datewhen noreadinprogress构造函数参数:
 如果为false，则刷新将直接传递给下一个处理程序;
 如果是正确的，则将下一个处理程序的调用作为事件循环的单独任务提交。在高吞吐量下，这使您有机会在执行任务之前处理其他刷新，从而将多个刷新打包为一个刷新。
 如果出现了明确的刷新，则刷新也将被转发(无论是在一个读循环中，还是在一个读循环的外部进行批处理)。
 如果通道变得不可写，它还将尝试执行任何挂起的刷新操作。
 flushconsolidated ationhandler应该放在ChannelPipeline中的第一个通道处理程序中，以达到最好的效果。
 */
public class FlushConsolidationHandler extends ChannelDuplexHandler {
    ...
    /**
     * Create new instance which explicit flush after 256 pending flush operations latest.创建一个新的实例，在最近的256个挂起刷新操作之后显式刷新。
     */
    ...
    /**
     * Create new instance which doesn't consolidate flushes when no read is in progress.创建一个新实例，当没有读操作时不会合并刷新。
     *
