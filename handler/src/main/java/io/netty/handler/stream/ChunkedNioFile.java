 ...
 * 使用NIO FileChannel通过块从文件块中获取数据的ChunkedInput。
 如果您的操作系统支持零拷贝文件传输，比如sendfile()，那么您可能希望使用FileRegion。
 */
public class ChunkedNioFile implements ChunkedInput<ByteBuf> {
    ...
    /**
     * Creates a new instance that fetches data from the specified file.创建一个新实例，该实例从指定的文件中获取数据。
     */
    ...
    /**
     * Returns the offset in the file where the transfer began.返回传输开始的文件中的偏移量。
     */
    ...
    /**
     * Returns the offset in the file where the transfer will end.返回传输结束的文件中的偏移量。
     */
    ...
    /**
     * Returns the offset in the file where the transfer is happening currently.返回当前发生传输的文件中的偏移量。
     */
