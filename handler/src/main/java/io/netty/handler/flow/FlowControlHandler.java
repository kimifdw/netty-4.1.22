 ...
 * FlowControlHandler确保每个read()只向下游发送一条消息。ByteToMessageDecoder或MessageToByteEncoder这样的类可以自由地为任何给定的输入输出任意多的事件。在这些场景中，通道的自动读取配置通常不适用。这在下游的ChannelHandlers中造成了问题，它们希望在处理一个事件时保存后续事件。HttpObjectDecoder有一个常见的问题，它通常会触发一个HttpRequest，然后紧接着是一个LastHttpContent事件。
 *
 */
public class FlowControlHandler extends ChannelDuplexHandler {
     ...
     * 确定底层队列是否为空。此方法用于测试、调试和检查，并不是线程安全的!
     */
    ...
    /**
     * Releases all messages and destroys the {@link Queue}.释放所有消息并销毁队列。
     */
    ...
    private static final class RecyclableArrayDeque extends ArrayDeque<Object> {
        ...
        /**
         * A value of {@code 2} should be a good choice for most scenarios.对于大多数情况，2的值应该是一个很好的选择。
         */
